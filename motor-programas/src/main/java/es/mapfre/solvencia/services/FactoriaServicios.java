package es.mapfre.solvencia.services;
...
/**
 * Factoría de Factorías. Cada una de las factorías de servicios ha de extender
 * esta clase.
 * 
 * @author Indra
 * 
 * @param <T>
 *            La implementación del servicio a buscar
 */
public abstract class FactoriaServicios<T extends Servicio> {
	...
	private void init(final Class<T> clazz) {
		...
		if (servicios == null) {
			...
			// Asignamos a la factoría el nuevo mapa de servicios
			...
			try {
				while (servicioIterator.hasNext()) {
					try {
						...
						// Le pedimos al servicio qué Servicio implementa
						String nombreServicio = servicio.getNombreServicio();
						// Si ya lo teníamos es un servicio con implementación
						// duplicada
		...
		for (Map.Entry<String, List<String>> servicioConflicto : serviciosDuplicados
			...
			StringBuilder mensaje = new StringBuilder("El servicio ")
					...
					.append(" tiene varias implementaciones, se usará la primera: ")
	...
	/**
	 * Método para acceder a los servicios registrados por una implementación de
	 * la factoría
	 * 
	...
	/**
	 * Guarda en la implementación de la factoría los servicios a registrar.
	 * 
	 * @param servicios
	...
	/**
	 * Devuelve todos los Servicios menos los Servicios indicados
	 * 
	 * @param nombreServicios
	 * @return los Servicios buscados
	 */
	protected List<T> getServiciosFiltrado(List<String> nombreServicios) {
		List<T> serviciosBuscados = new ArrayList<T>();
		Map<String, T> servicios = getServicios();
		for (Entry<String, T> entry : servicios.entrySet()) {
			if (nombreServicios == null || !nombreServicios.contains(entry.getKey())) {
				serviciosBuscados.add(entry.getValue());
			}
		}
