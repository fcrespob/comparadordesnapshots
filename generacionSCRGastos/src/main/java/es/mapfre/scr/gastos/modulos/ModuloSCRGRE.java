...
public class ModuloSCRGRE {

	private static final Logger LOG = LoggerFactory.getLogger(ModuloSCRGRE.class);
	...
	public void execute(String fecha) {
		...
		try {
			if (ModuloSCRGRE.LOG.isTraceEnabled()) {
				ModuloSCRGRE.LOG.trace("Inicio de execute en clase ModuloSCRGRE");
			}
			...
			if (null == valores || valores.isEmpty()){
				...
				inci.setFecCierre(fecha);
				inci.setCodigoRetorno("09");
				inci.setInfAmpliada("No existen datos de estr̩s para la fecha de cierre indicada.");
				gi.write(inci);	
				throw new Solvencia2Excepcion("No existen datos de estr̩s para la fecha de cierre indicada." , inci);
			}
			...
			ModuloSCRGRE.LOG.error(solvExc.getMessage(), solvExc);
