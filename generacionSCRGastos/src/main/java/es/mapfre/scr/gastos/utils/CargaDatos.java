...
import es.mapfre.scr.gastos.dao.services.FactoriaDao;
...
public class CargaDatos {
	...
	private static final String BEANIO_CONFIG_XML = "beanio/beanio-config-scrgto.xml";
	
	public void cargaDatosGeneral(String clave,String fecCierre, String rutaBase, String tipo)
		...
		String filePath;
		String streamName;
		String[] daos = btUtils.getCargaFicherosProperty(clave, tipo).split(",");
		for (int i = 0; i < daos.length; i++) {
			streamName = daos[i];
			filePath = btUtils.getCargaFicherosProperty(streamName, tipo);
