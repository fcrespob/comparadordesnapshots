...
public class TimeNullTypeHandler extends DateTypeHandler implements TypeHandler, ConfigurableTypeHandler {
	...
	public Timestamp parse(String text) throws TypeConversionException {
		...
		try {
			...
			throw new TypeConversionException(e.getMessage()+"\""+textTotal+"\" to format "+this.getPattern(),e); 
		}
	...
	public DateTypeHandler newInstance(Properties prop) throws IllegalArgumentException {
		TimeNullTypeHandler tnth = new TimeNullTypeHandler();
		tnth.setPattern(prop.getProperty(FORMAT_SETTING));
		return tnth;
	}
