...
public class DefinicionProcesosBeanIOTest extends TestCase{
	
	private static Logger log = LoggerFactory.getLogger(DefinicionProcesosBeanIOTest.class);

	private static final String BEANIO_CONFIG_XML = "beanio/beanio-config.xml";
	private static String dataFileName = "src/test/resources/NAS/CIERRES/CATALOGOS/CTLG_TB880DPR0.TXT";
	/**
	...
	public void testBeanIO() {
		...
		try {
			...
			in = new BeanIOReader(BEANIO_CONFIG_XML, dataFileName, "DPR0");
			
			List<DefinicionProcesos> procesos = new ArrayList<DefinicionProcesos>();
			
			try {

				DefinicionProcesos DefinicionProcesos = null;
				
				while ((DefinicionProcesos = (DefinicionProcesos) in.read()) != null) {
					log.debug("Proceso: {}", DefinicionProcesos.getCproceso());
					procesos.add(DefinicionProcesos);
					/*
