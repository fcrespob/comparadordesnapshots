...
public class ValoresCurvaTipoBeanIOTest extends TestCase{
	
	private static Logger log = LoggerFactory.getLogger(ValoresCurvaTipoBeanIOTest.class);

	private static final String BEANIO_CONFIG_XML = "beanio/beanio-config.xml";
	private static String dataFileName = "src/test/resources/NAS/CIERRES/CATALOGOS/GBT_TB340VCI0.TXT";
	/**
	...
	public void testBeanIO() {
		...
		try {
			...
			in = new BeanIOReader(BEANIO_CONFIG_XML, dataFileName, "VCI0");
			
			List<ValoresCurvaTipo> values = new ArrayList<ValoresCurvaTipo>();
			
			try {

				ValoresCurvaTipo value = null;
				
				while ((value = (ValoresCurvaTipo) in.read()) != null) {
					log.debug("CodCurvaTipos: {}", value.getCodCurvaTipos());
					values.add(value);
				}
