...
public class DatosPbTecnicaBeanIOTest extends TestCase{
	
	private static Logger log = LoggerFactory.getLogger(DatosPbTecnicaBeanIOTest.class);

	private static final String BEANIO_CONFIG_XML = "beanio/beanio-config.xml";
	private static String dataFileName = "src/test/resources/NAS/CIERRES/201312/CTEC/GENCARTE_AUX_PBTECNI.TXT";
	/**
	...
	public void testBeanIO() {
		...
		try {
			...
			in = new BeanIOReader(BEANIO_CONFIG_XML, dataFileName, "X880JI04");
			
			List<DatosPbTecnica> values = new ArrayList<DatosPbTecnica>();
			
			try {

				DatosPbTecnica value = null;
				
				while ((value = (DatosPbTecnica) in.read()) != null) {
					log.debug("Iarrastre: {}", value.getIarrastre());
					values.add(value);
				}
