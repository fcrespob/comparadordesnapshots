...
public class FlujosProbablesBeanIOTest extends TestCase{
	
	private static Logger log = LoggerFactory.getLogger(FlujosProbablesBeanIOTest.class);

	private static final String BEANIO_CONFIG_XML = "beanio/beanio-config.xml";
	private static String dataFileName = "src/test/resources/NAS/CIERRES/CATALOGOS/CTLG_TB880FLP0.TXT";
	...
	public void testBeanIO() {
		...
		try {
			...
			in = new BeanIOReader(BEANIO_CONFIG_XML, dataFileName, "FLP0");

			List<FlujosProbables> flujos = new ArrayList<FlujosProbables>();
			
			try {

				FlujosProbables flujo = null;
				
				while ((flujo = (FlujosProbables) in.read()) != null) {
					log.debug(flujo.getBasetecnica());
					flujos.add(flujo);
				}
			...
			// Asserts
			Assert.assertTrue(flujos.size()!=0);
		} catch (Exception e) {
			log.error("Error ", e);
		}
