...
public class DefinicionProgramasBeanIOTest extends TestCase{
	
	private static Logger log = LoggerFactory.getLogger(DefinicionProgramasBeanIOTest.class);

	private static final String BEANIO_CONFIG_XML = "beanio/beanio-config.xml";
	private static String dataFileName = "src/test/resources/NAS/CIERRES/CATALOGOS/CTLG_TB880DPG0.TXT";
	...
	public void testBeanIO() {
		...
		try {
			...
			in = new BeanIOReader(BEANIO_CONFIG_XML, dataFileName, "DPG");
			
			List<DefinicionProgramas> procesos = new ArrayList<DefinicionProgramas>();
			
			try {

				DefinicionProgramas DefinicionProgramas = null;
				
				while ((DefinicionProgramas = (DefinicionProgramas) in.read()) != null) {
					log.debug("Programa: {}", DefinicionProgramas.getKmodulo());
					procesos.add(DefinicionProgramas);
					/*
