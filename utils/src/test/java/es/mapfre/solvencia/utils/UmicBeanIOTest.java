...
public class UmicBeanIOTest extends TestCase{
	
	private static Logger log = LoggerFactory.getLogger(UmicBeanIOTest.class);

	private static final String BEANIO_CONFIG_XML = "beanio/beanio-config.xml";
	private static String dataFileName = "src/test/resources/NAS/CIERRES/201312/CTEC/GENCARTE_MAESTRO_UNIFICADO.txt";
		...
		try {
			...
			try {
				...
				while ((umic = (Umic) in.read()) != null) {
					log.debug(umic.getDatosGenerales().getKpoliza().toString());
					umics.add(umic);
				}
			} catch (InvalidRecordException ire) {
				log.error("Error parseando fichero: {}", ire.toString());
				fail("Hay errores en fichero");
			} 
			if (in != null) {
				in.close();
			}
			// Asserts
			Assert.assertTrue(umics.size()!=0);
		} catch (Exception e) {
			log.error("Error ", e);
		}
