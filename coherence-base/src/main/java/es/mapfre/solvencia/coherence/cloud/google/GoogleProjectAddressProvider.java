package es.mapfre.solvencia.coherence.cloud.google;
...
public class GoogleProjectAddressProvider extends CloudAddressProvider {

	/**
	 * The logger to use.
	 */
	private static final Logger logger = LoggerFactory.getLogger(GoogleProjectAddressProvider.class.getName());
	...
	public static void main(String[] args) {
		try {
			GoogleProjectAddressProvider grap = new GoogleProjectAddressProvider();
		} catch (IOException e) {
			//e.printStackTrace();
		}
	}

	public GoogleProjectAddressProvider() throws IOException {
		String regionName = System.getProperty(
		"tangosol.coherence.googleaddressprovider.region");//, Regions.US_EAST_1.getName());
		
		if (logger.isDebugEnabled()) {
			logger.debug("Initializing WKA list from Google Instances for region {}.", regionName);
		}
		...
		if (wkaAddressList == null || wkaAddressList.size() == 0) {
			try {
				...
				logger.error("Error initializing Google Client", ex);
				throw ex;
			}
		} else {
	 ...
	 *            default constructor.
	 */
	protected GoogleProjectAddressProvider(String dummyDifferentiator) {
	...
	private List<InetSocketAddress> generateWKAList() {
		...
		String tagName = System.getProperty("tangosol.coherence.googleaddressprovider.rolename", "COHERENCE_SERVER");
		String portString = System.getProperty("tangosol.coherence.googleaddressprovider.port", "8088");
