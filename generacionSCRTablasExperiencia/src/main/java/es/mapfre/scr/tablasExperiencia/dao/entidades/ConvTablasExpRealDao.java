...
import es.mapfre.scr.tablasExperiencia.dominio.keys.ConvTablasExpRealKey;
...
public class ConvTablasExpRealDao extends DaoBase {
	...
	public List<ConvTablasExpReal> obtenerTablasExp(String fecCierre, String bt)  {
		...
		Query query = getCache().createQuery().includeValues().addCriteria(kfdesdeAtt.le(fecCierre).and(fhastaAtt.gt(fecCierre)).and(kbasetecAtt.eq(bt)));
