...
public class TestProgramaPVI001 {
	
	private static final Logger LOG = LoggerFactory.getLogger(TestProgramaPVI001.class);
	...
	public static void end() {
		TestProgramaPVI001.LOG.debug("Fin pruebas del programa PVI001");
	}
	
	@SuppressWarnings("unchecked")
	@BeforeClass
	public static void init() {
		TestProgramaPVI001.LOG.debug("Inicio pruebas del programa PVI001");
		try {
			...
			TestProgramaPVI001.LOG.debug(e.getMessage(),e);
		} catch (Throwable e) {
			TestProgramaPVI001.LOG.debug(e.getMessage(),e);
		}
	}	
	...
	public void test() {
		...
		TestProgramaPVI001.LOG.debug("Inicio programa PVI001");
		...
		try {
			for (int i = 0; i < arrUmics.length; i++) {
				...
				UtilTestProcesos.obtenerFlujoFormulaCerrada(umic, fichaProceso, detalleBT, lstDetCorriente, ConstantsModulos.CTE_PROY_VIDA);
				UtilTestProcesos.obtenerFlujoProvi(umic, fichaProceso, detalleBT, lstDetCorriente, ConstantsModulos.CTE_PROY_VIDA);
			}
		} catch (Exception e) {
			TestProgramaPVI001.LOG.debug(e.getMessage(),e);
			fail();
		...
		TestProgramaPVI001.LOG.debug("Fin programa PVI001");
	}
}
