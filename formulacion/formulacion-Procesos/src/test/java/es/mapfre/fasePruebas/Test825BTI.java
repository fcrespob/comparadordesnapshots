...
public class Test825BTI {
	
	private static final String RUTA = "src/test/resources/CARTERA.txt";
	
	private static final Logger LOG = LoggerFactory.getLogger(Test825BTI.class);
...
//	@Test
	public void test() {
		...
		try {
			...
			final Umic dejar1Umic = lstUmics.get(3);
			lstUmics.clear();
			...
			// Ficha proceso
			fichaProceso.setCtipobt(ConstantsModulos.CTE_BTI);
			...
			Test825BTI.LOG.debug(e.getMessage(), e);
			fail();
		} catch (Throwable e) {
			Test825BTI.LOG.debug(e.getMessage(), e);
		}
