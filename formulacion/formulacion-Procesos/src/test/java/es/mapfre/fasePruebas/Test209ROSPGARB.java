...
public class Test209ROSPGARB {
	
	private static final String RUTA = "src/test/resources/CARTERA.txt";
	
	private static final Logger LOG = LoggerFactory.getLogger(Test209ROSPGARB.class);
	
//	@Test
	public void test() {
		//Variables locales
		List<Umic> lstUmics = null;
		String corrientes[] = {ConstantsModulos.CTE_PROY_VIDA, "PROY_FALL", "", "PROY_GTOS", "", ""};
		...
		try {
			...
			Test209ROSPGARB.LOG.debug(e.getMessage(), e);
			fail();
		} catch (Throwable e) {
			Test209ROSPGARB.LOG.debug(e.getMessage(), e);
		}
