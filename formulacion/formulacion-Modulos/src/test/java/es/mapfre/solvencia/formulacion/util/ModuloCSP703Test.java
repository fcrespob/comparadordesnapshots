...
/**
 * Clase que testea el Módulo CSP703 
 * 
 * @author ogperez
 *
 */
public class ModuloCSP703Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloCSP703Test.class);
	...
	public static void init() {
		ModuloCSP703Test.LOG.debug("Inicializando pruebas");
		
		daos();
	}
	
	@Test
	public void moduloCSP703Test(){
		Modulo moduloCSP703 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_CSP703);
				
		try{
			BigDecimal csp703 = (BigDecimal) moduloCSP703.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			
			System.out.println(csp703);
			
			TestCase.assertEquals(BigDecimal.valueOf(13265.6263), csp703.setScale(4, RoundingMode.HALF_DOWN));
			
		}catch (Exception e){
			ModuloCSP703Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	...
	private static void daos(){
		try{						
			...
			dcu.setPrima(BigDecimal.valueOf(3));
