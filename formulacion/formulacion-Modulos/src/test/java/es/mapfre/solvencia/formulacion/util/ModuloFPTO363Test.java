...
public class ModuloFPTO363Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloFPTO363Test.class);
	...
	public static void init() {
		ModuloFPTO363Test.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloFPTO363Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloFPTO363Test(){
		Modulo moduloFPTO363 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_FPTO363);
				
		try{
			BigDecimal fpto363 = (BigDecimal) moduloFPTO363.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			BigDecimal fpto363_2 = (BigDecimal) moduloFPTO363.execute(getProyUmic(), getBloqueCorriente2(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			System.out.println(fpto363);
			System.out.println(fpto363_2);

			TestCase.assertEquals(BigDecimal.ZERO, fpto363.setScale(0, RoundingMode.HALF_DOWN));
			TestCase.assertEquals(BigDecimal.valueOf(0.000632536), fpto363_2.setScale(9, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloFPTO363Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	...
	private Umic getUmic(){
		...
		Timestamp fecFin = new Timestamp(new GregorianCalendar(0, 0, 0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	private BloqueCorriente getBloqueCorriente2(){
		BloqueCorriente bloqueCorriente = new BloqueCorriente();
		Timestamp fecDev = UtilFechas.incrMeses(FDEVENGO, FDEVENGO, 1, false);
		
		bloqueCorriente.setFechaDevengo(fecDev);
