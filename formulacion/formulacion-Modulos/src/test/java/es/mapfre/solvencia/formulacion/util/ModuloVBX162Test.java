...
/**
 * Clase que testea el Módulo VBX162 
 * 
 * @author ogperez
 *
 */
public class ModuloVBX162Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVBX162Test.class);
...
//	private static final Timestamp FDEVENGO = new Timestamp(new GregorianCalendar(2014, 03,15, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	public static void init() {
		ModuloVBX162Test.LOG.debug("Inicializando pruebas");
		
		daos();
	}
	
	@Test
	public void moduloVBX162Test(){
		Modulo moduloVBX162 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_VBX162);
				
		try{
			TotalFlujoProyeccion proy = (TotalFlujoProyeccion) moduloVBX162.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			BigDecimal vbx162 = proy.getProvbtiproy();
			
			System.out.println(vbx162);
			
			TestCase.assertEquals(BigDecimal.valueOf(13265.6263), vbx162.setScale(4, RoundingMode.HALF_DOWN));
			
		}catch (Exception e){
			ModuloVBX162Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
...
//		datosGenerales.setCtipoaport("U");
		...
		rentas.setPrevrenta(BigDecimal.valueOf(0));
		rentas.setTempVit("V");
		rentas.setCpagrenta("2");
		rentas.setNpergaran(60);
		rentas.setRentini(BigDecimal.valueOf(0));
		duraciones.setNdursegano(0);
	...
	private DetalleBaseTecnica getBtcUmic(){
		...
		btcUmic.setGtorosspCap(BigDecimal.valueOf(0));
