...
/**
 * Clase que testea el Módulo VZG2C, 
 * @author ogperez
 *
 */

public class ModuloVZG2CTest{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVZG2CTest.class);
	...
	private static final Timestamp FDEVENGO2 = new Timestamp(new GregorianCalendar(2023, 7, 30, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	public static void init() {
		ModuloVZG2CTest.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloVZG2CTest.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloVZG2CTest(){
		Modulo moduloVZG2C = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_VZG2C);
				
		try{
			BigDecimal vzg2c = (BigDecimal) moduloVZG2C.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			BigDecimal vzg2c_2 = (BigDecimal) moduloVZG2C.execute(getProyUmic(), getBloqueCorriente2(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			System.out.println(vzg2c);
			System.out.println(vzg2c_2);

			TestCase.assertEquals(BigDecimal.ONE, vzg2c.setScale(0, RoundingMode.HALF_DOWN));
			TestCase.assertEquals(BigDecimal.valueOf(0.999367464), vzg2c_2.setScale(9, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloVZG2CTest.LOG.debug(e.getMessage(), e);
			fail();
		}
	...
	private BloqueCorriente getBloqueCorriente2(){
		BloqueCorriente bloqueCorriente = new BloqueCorriente();
		bloqueCorriente.setFechaDevengo(FDEVENGO2);
