...
/**
 * Clase que testea el Módulo PRI004, módulo de cálculo que devuelve ela prima en el periodo que le corresponda pagar al 
 * asegurado.
 * @author apedro
 *
 */

public class ModuloPRI004Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI004Test.class);
	...
	public static void init() {
		ModuloPRI004Test.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloPRI004Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloPRI004Test(){
		Modulo moduloPRI004 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_PRI004);
				
		try{
			BigDecimal pri004 = (BigDecimal) moduloPRI004.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			System.out.println(pri004);

			TestCase.assertEquals(BigDecimal.valueOf(412.49034), pri004.setScale(5, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloPRI004Test.LOG.debug(e.getMessage(), e);
			fail();
		}
