...
public class ModuloVRTA363Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVRTA363Test.class);
	...
	public static void init() {
		ModuloVRTA363Test.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloVRTA363Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloVRTA363Test(){
		Modulo moduloVRTA363 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_VRTA363);
				
		try{
			BigDecimal vrta363 = (BigDecimal) moduloVRTA363.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			BigDecimal vrta363_2 = (BigDecimal) moduloVRTA363.execute(getProyUmic(), getBloqueCorriente2(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			System.out.println(vrta363);
			System.out.println(vrta363_2);

			TestCase.assertEquals(BigDecimal.ONE, vrta363.setScale(0, RoundingMode.HALF_DOWN));
			TestCase.assertEquals(BigDecimal.valueOf(0.999367464), vrta363_2.setScale(9, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloVRTA363Test.LOG.debug(e.getMessage(), e);
			fail();
		}
