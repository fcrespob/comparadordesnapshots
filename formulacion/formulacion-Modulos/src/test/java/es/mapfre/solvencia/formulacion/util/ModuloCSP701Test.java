...
/**
 * Clase que testea el Módulo CSP701 
 * 
 * @author ogperez
 *
 */
public class ModuloCSP701Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloCSP701Test.class);
	...
	public static void init() {
		ModuloCSP701Test.LOG.debug("Inicializando pruebas");
		
		daos();
	}
	
	@Test
	public void moduloCSP701Test(){
		Modulo moduloCSP701 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_CSP701);
				
		try{
			BigDecimal csp701 = (BigDecimal) moduloCSP701.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			
			System.out.println(csp701);
			
			TestCase.assertEquals(BigDecimal.valueOf(13265.6263), csp701.setScale(4, RoundingMode.HALF_DOWN));
			
		}catch (Exception e){
			ModuloCSP701Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	...
	private static void daos(){
		try{						
			...
			dcu.setBt("BTI");
			dcu.setIteracion(iteracion);
			dcu.setGast(BigDecimal.valueOf(2));
			dcu.setGastp(BigDecimal.valueOf(3));
