...
/**
 * Clase que testea el Módulo CSP031, módulo de cálculo que devuelve la cuantía de fallecimiento de la garantía principal.
 * @author apedro
 *
 */

public class ModuloCSP031Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloCSP031Test.class);
	...
	public static final int modalidad = 375;
	public static final int garantia = 1;
	public static final String negocio = "I";
	
	@BeforeClass
	public static void init() {
		ModuloCSP031Test.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloCSP031Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloCSP031Test(){
		Modulo moduloCSP031 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_CSP031);
				
		try{
			BigDecimal csp031 = (BigDecimal) moduloCSP031.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			TestCase.assertEquals(BigDecimal.valueOf(14351.83623), csp031.setScale(5, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloCSP031Test.LOG.debug(e.getMessage(), e);
			fail();
		}
