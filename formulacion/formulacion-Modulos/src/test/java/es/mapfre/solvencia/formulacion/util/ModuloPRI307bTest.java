...
/**
 * Clase que testea el Módulo PRI307b
 * @author ogperez
 *
 */

public class ModuloPRI307bTest{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI307bTest.class);
	...
	public static void init() {
		ModuloPRI307bTest.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloPRI307bTest.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloPRI307bTest(){
		Modulo moduloPRI307b = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_PRI307B);
				
		try{
			BigDecimal pri307b = (BigDecimal) moduloPRI307b.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			System.out.println(pri307b);

			TestCase.assertEquals(BigDecimal.valueOf(144.24), pri307b.setScale(2, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloPRI307bTest.LOG.debug(e.getMessage(), e);
			fail();
		}
	...
	private Umic getUmic(){
		...
		Timestamp fecinisus = new Timestamp(new GregorianCalendar(1993, 12, 30, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		Timestamp fecdesderenova = new Timestamp(new GregorianCalendar(2014, 11, 30, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		Timestamp fechastarenova = new Timestamp(new GregorianCalendar(2014, 12, 30, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
...
//		datosGenerales.setCtipoaport("U");
		...
		datosGenerales.setTipoflexprim("N");
