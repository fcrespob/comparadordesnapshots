...
/**
 * Clase que testea el Módulo VBX164
 * 
 * @author ogperez
 *
 */
public class ModuloVBX164Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVBX164Test.class);
...
//	private static final Timestamp FDEVENGO = new Timestamp(new GregorianCalendar(2014, 03,15, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	public static void init() {
		ModuloVBX164Test.LOG.debug("Inicializando pruebas");
		
		daos();
	}
	
	@Test
	public void moduloVBX164Test(){
		Modulo moduloVBX164 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_VBX164);
				
		try{
			TotalFlujoProyeccion proy = (TotalFlujoProyeccion) moduloVBX164.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			BigDecimal vbx164 = proy.getProvbtiproy();
			
			System.out.println(vbx164);
			
			TestCase.assertEquals(BigDecimal.valueOf(13265.6263), vbx164.setScale(4, RoundingMode.HALF_DOWN));
			
		}catch (Exception e){
			ModuloVBX164Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
