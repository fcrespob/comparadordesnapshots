...
/**
 * Clase que testea el Módulo PRI001B, módulo de cálculo que devuelve ela prima en el periodo que le corresponda pagar al 
 * asegurado.
 * @author apedro
 *
 */

public class ModuloPRI001BTest{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI001BTest.class);
	...
	public static void init() {
		ModuloPRI001BTest.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloPRI001BTest.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloPRI001BTest(){
		Modulo moduloPRI001B = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_PRI001B);
				
		try{
			BigDecimal PRI001B = (BigDecimal) moduloPRI001B.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			System.out.println(PRI001B);

			TestCase.assertEquals(BigDecimal.valueOf(412.49034), PRI001B.setScale(5, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloPRI001BTest.LOG.debug(e.getMessage(), e);
			fail();
		}
	...
	private List<DetalleCorriente> getProyUmic(){
		...
		for (int i=0;i<85;i++){
			...
			bloque.setImpFlujoNominal(BigDecimal.ZERO);
