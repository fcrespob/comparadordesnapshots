...
/**
 * Clase que testea el Módulo VRTA162, módulo de cálculo que devuelve los factores de probabilización a aplicar en  el 
 * cálculo de la cuantía probable de la garantía.
 * @author apedro
 *
 */

public class ModuloVRTA162Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVRTA162Test.class);
	...
	private static final Timestamp FDESDE = new Timestamp(new GregorianCalendar(2014, 12, 31, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	private static final Timestamp FDEVENGO = new Timestamp(new GregorianCalendar(2024, 03, 30, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	private static final int iteracion = 11;
	...
	public static void init() {
		ModuloVRTA162Test.LOG.debug("Inicializando pruebas");
		...
		try{
			...
			ModuloVRTA162Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	
	@Test
	public void moduloVRTA162Test(){
		Modulo moduloVRTA162 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_VRTA162);
				
		try{
			BigDecimal vrta162 = (BigDecimal) moduloVRTA162.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			System.out.println(vrta162);
			//TestCase.assertEquals(BigDecimal.valueOf(1), vrta162);
			TestCase.assertEquals(BigDecimal.valueOf(0.945255686), vrta162.setScale(9, RoundingMode.HALF_DOWN));
		}catch (Exception e){
			ModuloVRTA162Test.LOG.debug(e.getMessage(), e);
			fail();
		}
