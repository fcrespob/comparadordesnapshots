...
/**
 * Clase que testea el Módulo VBX462
 * 
 * @author ogperez
 *
 */
public class ModuloVBX462Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVBX462Test.class);
...
//	private static final Timestamp FHASTA = new Timestamp(new GregorianCalendar(2015, 01, 8, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	private static final Timestamp FPAGO = new Timestamp(new GregorianCalendar(2014, 02,31, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	public static void init() {
		ModuloVBX462Test.LOG.debug("Inicializando pruebas");
		
		daos();
	}
	
	@Test
	public void moduloVBX462Test(){
		Modulo moduloVBX462 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_VBX462);
				
		try{
			TotalFlujoProyeccion proy = (TotalFlujoProyeccion) moduloVBX462.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			BigDecimal vbx462 = proy.getProvbtiproy();
			
			System.out.println(vbx462);
			
			TestCase.assertEquals(BigDecimal.valueOf(13265.6263), vbx462.setScale(4, RoundingMode.HALF_DOWN));
			
		}catch (Exception e){
			ModuloVBX462Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
...
//		datosGenerales.setCtipoaport("U");
		...
		primas.setIprimatarada(BigDecimal.valueOf(100));
		bti.setPgastgesex1I(BigDecimal.ONE);
	...
	private List<DetalleCorriente> getProyUmic(){
		...
		for (int i=0;i<155;i++){
			...
			detalleCorriente.setImpPago(BigDecimal.ONE);
	...
	private DetalleBaseTecnica getBtcUmic(){
		...
		btcUmic.setGtorosspPrima(BigDecimal.ONE);
