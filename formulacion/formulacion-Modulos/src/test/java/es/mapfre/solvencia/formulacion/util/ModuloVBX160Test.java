...
/**
 * Clase que testea el Módulo VBX160 
 * 
 * @author apedro
 *
 */
public class ModuloVBX160Test{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVBX160Test.class);
...
//	private static final String terminal = "VTX003";
	...
	public static void init() {
		ModuloVBX160Test.LOG.debug("Inicializando pruebas");
		
		daos();
	}
	
	@Test
	public void moduloVBX100Test(){
		Modulo moduloVBX160 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_VBX160);
				
		try{
			TotalFlujoProyeccion proy = (TotalFlujoProyeccion) moduloVBX160.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			BigDecimal vbx160 = proy.getProvbtiproy();
			
			System.out.println(vbx160);
			
			TestCase.assertEquals(BigDecimal.valueOf(13265.6263), vbx160.setScale(4, RoundingMode.HALF_DOWN));
			
		}catch (Exception e){
			ModuloVBX160Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
