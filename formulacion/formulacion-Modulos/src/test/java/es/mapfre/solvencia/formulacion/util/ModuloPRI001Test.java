...
public class ModuloPRI001Test {

private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI001Test.class);
	...
	public static void init() {
		ModuloPRI001Test.LOG.debug("Inicializando pruebas");
		
		daos();
	}
	
	@Test
	public void moduloPRI001Test(){
		Modulo moduloPRI001 = FactoriaModulos.getModulo(ConstantsFactorias.MODULO_PRI001);
				
		try{
			BigDecimal PRI001 = (BigDecimal) moduloPRI001.execute(getProyUmic(), getBloqueCorriente(), iteracion, FCALC, getUmic(), getBtcUmic(), mapVariables, codigoSubproceso);
			
			
			System.out.println(PRI001);
			
			TestCase.assertEquals(BigDecimal.valueOf(13265.6263), PRI001.setScale(4, RoundingMode.HALF_DOWN));
			
		}catch (Exception e){
			ModuloPRI001Test.LOG.debug(e.getMessage(), e);
			fail();
		}
	}
	...
	private Umic getUmic(){
		...
		datosGenerales.setCcanal(null);
		datosGenerales.setTipoSubriesgo(null);
		datosGenerales.setKcategoria(null);
		umic.setDatosGenerales(datosGenerales);
		
		Timestamp fecdesderenova = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		Timestamp fechastarenova = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		Timestamp fecefecred = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		Timestamp fecinisus = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		Timestamp fecinipagprim = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		Timestamp fecefecini = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		...
		bti.setPgastgesex1I(null);
		bti.setPsobremort(null);
		bti.setPriesgo(null);
		umic.setBti(bti);
		
		Timestamp fecIni = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
	...
	private List<DetalleCorriente> getProyUmic(){
		...
		bloqueFall.setImpFlujoNominal(null);
	...
	private BloqueCorriente getBloqueCorriente(){
		...
		Timestamp fechaPago = new Timestamp(new GregorianCalendar(2014, Calendar.MARCH, 01, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0, ConstantsFunciones.CTE_0).getTimeInMillis());
		bloqueCorriente.setFechaPago(fechaPago);
	...
	private DetalleBaseTecnica getBtcUmic(){
		...
		itcalc.add(BigDecimal.valueOf(0.0));
		...
		btcUmic.setTablacalc1aseg1(null);
		btcUmic.setIndTabExp('a');
