 ...
 *
 */
public class ModuloACTBEL implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory
			.getLogger(ModuloACTBEL.class);

	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_ACTBEL;

	private static final String CLAVE_CRIFEC = ConstantsModulos.CTE_VA_CRIT_FEC;
	private static final String CLAVE_VAR_CRIFEC = CLAVE_CRIFEC
			.concat(CLAVE_MODULO);

	private static final String CLAVE_VA_INTER = ConstantsModulos.CTE_VA_INTER;
	private static final String CLAVE_VAR_CRIINT = ConstantsModulos.CTE_VA_INTER
			.concat(CLAVE_MODULO);
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloACTBEL.LOG.isTraceEnabled()) {
				ModuloACTBEL.LOG
						.trace("Inicio de execute en clase ModuloACTBEL");
			}
			...
			resultado = moduloACTBEL(proyUmic, bloqueCorriente, iteracion,
					fcalc, umic, btcUmic, mapVariables);

		} catch (Solvencia2Excepcion e) {
			ModuloACTBEL.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloACTBEL.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(
					ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}

		if (ModuloACTBEL.LOG.isTraceEnabled()) {
			ModuloACTBEL.LOG.trace("Fin de execute en clase ModuloACTBEL");
		}
	 ...
	 */
	private BigDecimal moduloACTBEL(final List<DetalleCorriente> proyUmic,
		...
		BigDecimal actbel = BigDecimal.ZERO;
		...
		if (ModuloACTBEL.LOG.isTraceEnabled()) {
			ModuloACTBEL.LOG
					.trace("Inicio función << moduloACTBEL >> de la clase ModuloACTBEL, para la iteracion = {}",
							iteracion);
		}
		...
		varCriterInt = UtilModulos.getsetStringRecuperarDefinicionAuxiliar(
				mapVariables, CLAVE_VAR_CRIINT, umic.getDatosGenerales()
						.getCcartera(), umic.getDatosGenerales()
						.getKmodalidad(), umic.getDatosGenerales()
						.getKgarantia(), btcUmic.getBaseTec(), CLAVE_VA_INTER);
		...
		if (ConstantsModulos.CTE_FIRST_ITER.equals(iteracion)) {
			ValidacionesComunesModulos
					.validarVariableDeApoyoVarCriterioFecha(varCriterFec);
			ValidacionesComunesModulos
					.validarVariableDeApoyoVarCritInt(varCriterInt);
		...
		// varValoresCurva =
		// obtenerConfiguracion.recuperarValoresTipos(btcUmic.ktipobt, fcalc,
		// umic.DatosGenerales.carterainv)
		
//PYAM0001-TAR00163093-Modificación_ACTBEL-INI
		// A la hora de recuperar los valores de las curvas TI, tenemos que tener en cuenta si estamos en periodo casado o no.
		//1.	Si la umic es CASADA en fcal:
		//		i.	Si estamos en periodo casado  usa la curva que recupera de la tabla
		//		ii.	Si estamos en periodo no casado  usa la CLR000
		//2.	Si la umic es NO CASADA en fcal  usa la curva que recupera de la tabla

		//  
		String casadoX = ConstantsFunciones.CTE_CADENA_VACIA;
		String Casado_Umic = umic.getDatosGenerales().getSwcasado();
		
		varValoresCurvaIni = UtilModulos.setGetListValoresCurvaTipo(mapVariables,
				CLAVELISTACURTI, btcUmic.getCurvaTi(), btcUmic.getFcurvaTi());
		...
		/*if (Casado_Umic == "N") {
			...
			casadoX = obtenerFechasIniFinTramoParaCalculoProvi(btcUmic,
				bloqueCorriente.getFechaPago());
			
			if (ConstantsModulos.CTE_N.equals(casadoX)) {
				if (btcUmic.getBt().equals(ConstantsModulos.CTE_VAL_SCRTID)){
					varValoresCurvaIni = UtilModulos.setGetListValoresCurvaTipo(mapVariables,
							CLAVELISTACURTI,ConstantsModulos.CURVA_CLR000D, btcUmic.getFcurvaTi());
				} else if (btcUmic.getBt().equals(ConstantsModulos.CTE_VAL_SCRTIU)) {
					varValoresCurvaIni = UtilModulos.setGetListValoresCurvaTipo(mapVariables,
							CLAVELISTACURTI,ConstantsModulos.CURVA_CLR000U, btcUmic.getFcurvaTi());
				} else {
					varValoresCurvaIni = UtilModulos.setGetListValoresCurvaTipo(mapVariables,
							CLAVELISTACURTI,ConstantsModulos.CURVA_CLR0000, btcUmic.getFcurvaTi());
				}
			}else {
				varValoresCurvaIni = UtilModulos.setGetListValoresCurvaTipo(mapVariables,
						CLAVELISTACURTI, btcUmic.getCurvaTi(), btcUmic.getFcurvaTi());
...
//PYAM0001-TAR00163093-Modificación_ACTBEL-FIN
		...
		if (elementosLstCurTipo != 0) {
			maximoDiasPLazo = varValoresCurvaIni.get(elementosLstCurTipo - 1)
					.getDiasPlazo();
			maximoPorcentajeInteres = varValoresCurvaIni.get(
					elementosLstCurTipo - 1).getPorcentajeInteres();
		}
		...
		varDurJ = FuncionesAuxiliares.nDias(fcalc,
				bloqueCorriente.getFechaPago(), varCriterFec);
		if (varDurJ == ConstantsFunciones.CTE_0) {
			varDurJ = ConstantsFunciones.CTE_1;
		}
		
		/**
		 * Si  btcUmic.btcalc = 'BEL' y (btcUmic.txt_per_trans = 'S'  Y  btcUmic.num_factor_interpo_it_bel <> 0  ):
		 * btcUmicROSSP = (btcUmic.fecCierre,’ROSSP’,claveUmic)
		 * Con esta fecha buscaremos en los datos de la umic el tramo de intenreés en el que se sitúa dicha fecha, que será el tramo que cumpla las siguientes condiciones:
		 * 		btcUmicROSSP. fecIniTramox <= btcUmic.fec_curva_ti < btcUmicROSSP. fecFinTramoX
		 * Para el tramo así obtenido recuperamos el porcentaje de interés ROSSP antes recuperado:
		 * 		varIctROSSP = btcUmicROSSP.NUM_ITCALC_X
		 * Por otro lado, a la curva de valores recuperada se le aplicará el factor de interpolación BEL (btcUmic.num_factor_interpo_it_bel), de forma que, para todo j de la tabla se hará:
		 * 		varValoresCurva(j).porcInteres = varValoresCurva(j).porcInteres + (btcUmic.num_factor_interpo_it_bel * (varIctROSSP – varValoresCurva(j).porcInteres
		 * 
		 * */
			...
			if ((btcUmic.getBt().equals(ConstantsModulos.CTE_BT_BEL) || 
				...
				if (btcUmic.getPerTransBel() && btcUmic.getFactorInterpolInteresesBel().signum()!=0){
					...
					if(btcUmic.getBt().equals(ConstantsModulos.CTE_BT_BEL) || 
							...
							btcUmic.getBt().equals(ConstantsModulos.CTE_VAL_SCRAIN) || 
							btcUmic.getBt().equals(ConstantsModulos.CTE_VAL_SCRANM)){
						btcUmicROSSP = obtenerDatos.recuperarBTCUmic(btcUmic.getFecCierre(), ConstantsModulos.CTE_BT_ROSSP, umic.getKey());
					}else if(btcUmic.getBt().equals(ConstantsModulos.CTE_VAL_BELCOA)){
						btcUmicROSSP = obtenerDatos.recuperarBTCUmic(btcUmic.getFecCierre(), ConstantsModulos.CTE_VAL_BTCOA, umic.getKey());
					}else if(btcUmic.getBt().equals(ConstantsModulos.CTE_VAL_BELCLR)){
						btcUmicROSSP = obtenerDatos.recuperarBTCUmic(btcUmic.getFecCierre(), ConstantsModulos.CTE_VAL_ROSSP, umic.getKey());
				 	}
					...
					for(int i=0; i<btcUmicROSSP.getFecInitramo().size() && btcUmicROSSP.getFecInitramo().get(i)!=null;i++){
						if (!(btcUmicROSSP.getFecInitramo().get(i).after(fCurvaTi)) && btcUmicROSSP.getFecfintramo().get(i).after(fCurvaTi)){
					...
					for(ValoresCurvaTipo valorC: varValoresCurva){
						BigDecimal interes = valorC.getPorcentajeInteres().add((BigDecimal.ONE.subtract(btcUmic.getFactorInterpolInteresesBel())).multiply(varItcROSSP.subtract(valorC.getPorcentajeInteres())));
		...
		if (varDurJ >= maximoDiasPLazo.intValue()) {
			...
			varIct = FuncionesActualizacionFinanciera.calcularTipoInteres(varCriterInt,
					varDurAnt, varIntAnt, varDurPost, varIntPost, varDurJ);
		}
		...
		actbel = Util.pow(BigDecimal.ONE.add(varIct.divide(
				ConstantsFunciones.CTE_OPER_100,
				ConstantsFunciones.MATH_CONTEXT)),
				FuncionesAuxiliares.nAnnos(fcalc, varFecJ, varCriterFec)
						.negate());

		if (ModuloACTBEL.LOG.isTraceEnabled()) {
			ModuloACTBEL.LOG
					.trace("Fin función << moduloACTBEL >> de la clase ModuloACTBEL, para la iteracion = {}, con resultado actbel = {}",
							iteracion, actbel);
		}
	...
	private int obtenerIndicePlazoAnterior(
		...
		if (ModuloACTBEL.LOG.isTraceEnabled()) {
			ModuloACTBEL.LOG
					.trace("Fin de la función << obtenerIndicePlazoAnterior >> de la clase ModuloACTBEL");
		}
	 ...
	 * @return valorDevolver
	 */
	private ValoresCurvaTipo obtenerPlazoPosterior(
			final List<ValoresCurvaTipo> lstValCurvaTipo, final Integer varDurJ) {
		// Variables locales
		...
		if (ModuloACTBEL.LOG.isTraceEnabled()) {
			ModuloACTBEL.LOG
					.trace("Inicio de la función << obtenerPlazoPosterior >> de la clase ModuloACTBEL");
		}
		...
		if (ModuloACTBEL.LOG.isTraceEnabled()) {
			ModuloACTBEL.LOG
					.trace("Fin de la función << obtenerPlazoPosterior >> de la clase ModuloACTBEL");
		}
	...
	/**
	 * Función encargada de copiar la lista de ValoresCurvaTipo recuperada a una nueva lista (para no sobreescribir la lista original en las operaciones)
	 * @param valores Lista a copiar
	...
	private List<ValoresCurvaTipo> copiarValoresCurvaTipo(final List<ValoresCurvaTipo> valores){
		...
		for (ValoresCurvaTipo valorC: valores){
			...
			try {
				...
				ModuloACTBEL.LOG.error(e.getMessage());
			}
...
//PYAM0001-TAR00163093-Modificación_ACTBEL-INI
	 ...
	 * @return casadoX
	 */
	private static String obtenerFechasIniFinTramoParaCalculoProvi(
			final DetalleBaseTecnica btcUmic, final Timestamp fcalc) {
		// Variables locales
		String resultado = ConstantsFunciones.CTE_CADENA_VACIA;
		// int diasFecIniFCal;
		// int diasFCalFFin;
		boolean noEncontrado = true;
		final Timestamp fecIniTramo[] = { btcUmic.getFecInitramo().get(0),
				btcUmic.getFecInitramo().get(1),
				btcUmic.getFecInitramo().get(2),
				btcUmic.getFecInitramo().get(3),
				btcUmic.getFecInitramo().get(4) };
		final Timestamp fecFinTramo[] = { btcUmic.getFecfintramo().get(0),
				btcUmic.getFecfintramo().get(1),
				btcUmic.getFecfintramo().get(2),
				btcUmic.getFecfintramo().get(3),
				btcUmic.getFecfintramo().get(4) };
		final String casadoX[] = { btcUmic.getSwcasado().get(0),
				btcUmic.getSwcasado().get(1), btcUmic.getSwcasado().get(2),
				btcUmic.getSwcasado().get(3), btcUmic.getSwcasado().get(4) };
			...
			for (int i = 0; i < longitudVector && noEncontrado; i++) {
				if (null != fecIniTramo[i] && null != fecFinTramo[i]) {
					...
					if ((!fcalc.before(fecIniTramo[i]) && fecFinTramo[i].after(fcalc)) || 
							((!fcalc.before(fecIniTramo[i]) && !fecFinTramo[i].before(fcalc)) && ((i<longitudVector-1 && fecIniTramo[i+1] == null) || i==longitudVector-1))) {
						resultado = casadoX[i];
						noEncontrado = false;
					}
				}
