 ...
 *
 */
public class ModuloCOMPLIPTOZC implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloCOMPLIPTOZC.class);

	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_COMPLIPTOZC;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloCOMPLIPTOZC.LOG.isTraceEnabled()) {
				ModuloCOMPLIPTOZC.LOG.trace("Inicio de execute en clase COMPLIPTOZC");
			}
			...
			resultado = moduloCOMPLIPTOZC(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloCOMPLIPTOZC.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloCOMPLIPTOZC.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloCOMPLIPTOZC.LOG.isTraceEnabled()) {
			ModuloCOMPLIPTOZC.LOG.trace("Fin de execute en clase COMPLIPTOZC");
		}
	 ...
	 */
	private BigDecimal moduloCOMPLIPTOZC(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		BigDecimal varLizcnanosant = BigDecimal.ZERO;
		//Fin variables locales
		
		if (ModuloCOMPLIPTOZC.LOG.isTraceEnabled()) {
			ModuloCOMPLIPTOZC.LOG.trace("Inicio función << moduloCOMPLIPTOZC >> de la clase moduloCOMPLIPTOZC, para la iteracion = {}", iteracion);
		}
		...
		jant = (Timestamp) mapVariables.get("FECHA_DEVENGO_COMPLIPTOZC");
		...
		if(null == varBtcUmic2){
			...
			try {
				...
				ModuloCOMPLIPTOZC.LOG.error(e.getMessage());
			}
		...
		varLizcnanosant = UtilModulos.getVarLzcVZC(mapVariables, CLAVE_VAR_LIZC + varZc + nanos_jant + varTabMort + umic.getAsegurados().getFnacAseg1(), varZc.add(nanos_jant), varValoresLix);
		
		fptoizc_t1 = varLzcnanos.divide(varLzc, ConstantsFunciones.MATH_CONTEXT);
		fptoizc_t2 = (varLizcnanosant.subtract(varLizcnanos)).divide(varLizc, ConstantsFunciones.MATH_CONTEXT);
		fptoizc = fptoizc_t1.multiply(fptoizc_t2);
		
		if (ModuloCOMPLIPTOZC.LOG.isTraceEnabled()) {
			ModuloCOMPLIPTOZC.LOG.trace("Fin función << ModuloCOMPLIPTOZC >> de la clase ModuloCOMPLIPTOZC, para la iteracion = {}, con resultado compliptozc = {}", iteracion, fptoizc);
		}
		mapVariables.put("FECHA_DEVENGO_COMPLIPTOZC", bloqueCorriente.getFechaDevengo());
		return fptoizc;
	}
