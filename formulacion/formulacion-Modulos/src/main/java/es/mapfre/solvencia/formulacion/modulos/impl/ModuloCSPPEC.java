 ...
 *
 */
public class ModuloCSPPEC implements Modulo {

	// Inicio de las variables estáticas para agilizar operaciones.
	private static final Logger LOG = LoggerFactory.getLogger(ModuloCSPPEC.class);
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_CSPPEC;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloCSPPEC.LOG.isTraceEnabled()) {
				ModuloCSPPEC.LOG.trace("Inicio de execute en clase ModuloCSPPEC");
			}
			...
			resultado = moduloCSPPEC(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloCSPPEC.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloCSPPEC.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloCSPPEC.LOG.isTraceEnabled()) {
			ModuloCSPPEC.LOG.trace("Fin de execute en clase ModuloCSPPEC");
		}
	 ...
	 */
	private BigDecimal moduloCSPPEC(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		BigDecimal csppec = BigDecimal.ZERO;
		BigDecimal varMultiplicando;
		//Fin variables locales
		
		if (ModuloCSPPEC.LOG.isTraceEnabled()) {
			ModuloCSPPEC.LOG.trace("Inicio función << moduloCSPPEC >> de la clase ModuloCSPPEC, para la iteracion = {}", iteracion);
		}
		...
		//varPUCCAPcol = FuncionesVBX.puccapcol(varPPCAP, varN, varI1Bti, varI2Bti, varNR, varNRM, varM, varB1, varB2, proyUmic.get(iteracion-1).getFechaDesde(), umic.getBti().getFecFinTramo1(), mapVariables, CLAVE_MODULO);
		
		varMultiplicando = BigDecimal.ONE.add(varI1Bti);
		
		csppec = varPPCAP.multiply(varMultiplicando.pow(varNR));
		
		if (ModuloCSPPEC.LOG.isTraceEnabled()) {
			ModuloCSPPEC.LOG.trace("Fin función << moduloCSPPEC >> de la clase ModuloCSPPEC, para la iteracion = {}, con resultado csppec = {}", iteracion, csppec);
		}
