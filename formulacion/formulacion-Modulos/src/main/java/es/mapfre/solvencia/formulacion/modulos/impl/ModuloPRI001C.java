...
public class ModuloPRI001C implements Modulo{
	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI001C.class);
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI001C;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI001C.LOG.isTraceEnabled()) {
				ModuloPRI001C.LOG.trace("Inicio de execute en clase ModuloPRI001C");
			}
			...
			resultado = moduloPRI001C(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, codSubproceso ,mapVariables);
		} catch (Solvencia2Excepcion e) {
			ModuloPRI001C.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI001C.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		if (ModuloPRI001C.LOG.isTraceEnabled()) {
			ModuloPRI001C.LOG.trace("Fin de execute en clase ModuloPRI001C");
		}
		return resultado;
	}
	/**
	...
	private BigDecimal moduloPRI001C(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		//Fin variables locales
		if (ModuloPRI001C.LOG.isTraceEnabled()) {
			ModuloPRI001C.LOG.trace("Inicio de la función << moduloPRI001C >> de la clase ModuloPRI001C, para la iteración = {}", iteracion);
		}
		...
		if(!(null == bloqueCorriente.getFechaPago())){
			...
			if(bloqueCorriente.getFechaPago().after(umic.getFechas().getFecefecfin())){
				...
				pri001C = umic.getPrimas().getIprimatarada().multiply(BigDecimal.ONE.add(umic.getPrimas().getPrecargfrac().multiply(ConstantsFunciones.CTE_OPER_0_PUNTO_01))).divide(varNP, ConstantsFunciones.MATH_CONTEXT);
			}
		}
		if (ModuloPRI001C.LOG.isTraceEnabled()) {
			ModuloPRI001C.LOG.trace("Fin de la función << moduloPRI001C >> de la clase ModuloPRI001C, para la iteración = {}", iteracion);
		}
		return pri001C;
	}
}
