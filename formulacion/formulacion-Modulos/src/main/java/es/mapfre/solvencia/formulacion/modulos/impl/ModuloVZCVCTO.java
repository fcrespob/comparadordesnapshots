 ...
 *
 */
public class ModuloVZCVCTO implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVZCVCTO.class);

	// Estas variables se ponen como estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_VZCVCTO;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloVZCVCTO.LOG.isTraceEnabled()) {
				ModuloVZCVCTO.LOG.trace("Inicio de execute en clase ModuloVZC");
			}
			...
			resultado = moduloVZCVCTO(proyUmic, bloqueCorriente, iteracion, fcalc,
					umic, btcUmic, mapVariables, codSubproceso);

		} catch (Solvencia2Excepcion e) {
			ModuloVZCVCTO.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloVZCVCTO.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(
					ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}

		if (ModuloVZCVCTO.LOG.isTraceEnabled()) {
			ModuloVZCVCTO.LOG.trace("Fin de execute en clase ModuloVZC");
		}
	 ...
	 * @return vzcj
	 */
	private BigDecimal moduloVZCVCTO(final List<DetalleCorriente> proyUmic,
		...
		BigDecimal varFraccj2 = BigDecimal.ZERO;
		...
		//varLzc = UtilModulos.getVarLzcVZC(mapVariables, CLAVE_VAR_LZC + varZc + varTabMort + fecNacAseg, varZc.add(varFraccj), lstValoresTabMort);
		if (varZc.add(varFraccj).intValue() > lstValoresTabMort.size()-2){
			varLzc = BigDecimal.ZERO;
		}else{
			BigDecimal varLzcEntero = lstValoresTabMort.get(varZc.add(varFraccj).intValue());
			BigDecimal varLzcEntero1 = lstValoresTabMort.get(varZc.add(varFraccj).intValue() + 1);
			//salida = varLzcEntero.add(varZc.remainder(BigDecimal.ONE).multiply(varLzcEntero1.subtract(varLzcEntero)));
			varLzc = Util.interpolaPorEdad(varLzcEntero, varLzcEntero1, varZc.add(varFraccj));
		}
		// Estos valores SÍ VARÍAN con cada iteración.

		

		varFraccj2 = FuncionesAuxiliares.nAnnos(bloqueCorriente.getFechaDevengo(),
				proyUmic.get(proyUmic.size()-1).getFechaHasta(), varCriterioFec);
		
//INI-TAR00400971
		if ((umic.getDatosGenerales().getKprestacion() != null) && (umic.getDatosGenerales().getKprestacion().equals("RS901"))) {
			varFraccj = FuncionesAuxiliares.nAnnos(fcalc,
					proyUmic.get(iteracion).getFechaDesde(), varCriterioFec);	
			}
//FIN-TAR00400971
		varEdadJ = varZc.add(varFraccj).add(varFraccj2);
		varEdadJ = varEdadJ.setScale(15, RoundingMode.HALF_UP);
		...
		if (ModuloVZCVCTO.LOG.isTraceEnabled()) {
			ModuloVZCVCTO.LOG.trace("Fin función << moduloVZC >> de la clase ModuloVZC, para la iteracion = {},  con resultado capitalCartera = {}",
						iteracion, vzcJ);
		}
