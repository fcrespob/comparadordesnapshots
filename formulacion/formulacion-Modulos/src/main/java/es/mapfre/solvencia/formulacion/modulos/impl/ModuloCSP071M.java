 ...
 *
 */
public class ModuloCSP071M implements Modulo {

	// Inicio de las variables estáticas para agilizar operaciones.
	private static final Logger LOG = LoggerFactory.getLogger(ModuloCSP071M.class);
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_CSP071M;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloCSP071M.LOG.isTraceEnabled()) {
				ModuloCSP071M.LOG.trace("Inicio de execute en clase ModuloCSP071");
			}
			...
			resultado = moduloCSP071M(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables,codSubproceso);

			
		} catch (Solvencia2Excepcion e) {
			ModuloCSP071M.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloCSP071M.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloCSP071M.LOG.isTraceEnabled()) {
			ModuloCSP071M.LOG.trace("Fin de execute en clase ModuloCSP071");
		}
	 ...
	 */
		...
		private BigDecimal moduloCSP071M(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		if (ModuloCSP071M.LOG.isTraceEnabled()) {
			ModuloCSP071M.LOG.trace("Inicio función << moduloCSP071 >> de la clase ModuloCSP071, para la iteracion = {}", iteracion);
		}
...
//		Se establecerá varfecini como la fecha de Efecto Técnico, es decir, como el fin de mes del mes de efecto de la UMIC (fechasUmic.fecinisus)
		...
		varD = FuncionesAuxiliares.nDias(umic.getFechas().getFecinisus(), umic.getFechas().getFecefecini(), varCriterFecha);
		...
		varLm = UtilModulos.getVarLm(mapVariables, CLAVE_VARL, ConstantsModulos.CTE_FIRST_ITER, umic.getBti().getFecIniTramo1(), umic.getBti().getFecFinTramo1(), varCriterFecha);
...
//			varPP = MOD(varTCm + varβ, 12/ varM) 
		...
		if (ModuloCSP071M.LOG.isTraceEnabled()) {
			ModuloCSP071M.LOG.trace("Fin función << moduloCSP071 >> de la clase ModuloCSP071, para la iteracion = {}, con resultado csp071 = {}", iteracion, csp071M);
		}
	...
	private BigDecimal calcularVarImporteRentaCasoVarTmcMayorCeroYPPIgualCero(final BigDecimal varRenta0, final BigDecimal varPrr, final Integer varTCm, 
		...
		if (ModuloCSP071M.LOG.isTraceEnabled()) {
			ModuloCSP071M.LOG.trace("Inicio de la función << calcularVarImporteRentaCasoVarTmcMayorCeroYPPIgualCero >> de la clase ModuloCSP071M");
		}
		...
		if (ModuloCSP071M.LOG.isTraceEnabled()) {
			ModuloCSP071M.LOG.trace("Fin de la función << calcularVarImporteRentaCasoVarTmcMayorCeroYPPIgualCero >> de la clase ModuloCSP071M");
		}
	...
	private Integer obtenerVarM(final Rentas rentasUmic) {
		...
		if (ModuloCSP071M.LOG.isTraceEnabled()) {
			ModuloCSP071M.LOG.trace("Inicio de la función << obtenerVarM >> de la clase ModuloCSP071");
		}
		...
		if (ModuloCSP071M.LOG.isTraceEnabled()) {
			ModuloCSP071M.LOG.trace("Fin de la función << obtenerVarM >> de la clase ModuloCSP071, con resultado varM = {}", varM);
		}
