...
public class ModuloVRTAVAS400 implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVRTAVAS400.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_VRTAVAS400;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloVRTAVAS400.LOG.isTraceEnabled()) {
				ModuloVRTAVAS400.LOG.trace("Inicio de execute en clase ModuloVRTAVAS400");
			}
			...
			//Invocamos a la función ModuloVRTAVAS400
			resultado = moduloVRTAVAS400(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloVRTAVAS400.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloVRTAVAS400.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloVRTAVAS400.LOG.isTraceEnabled()) {
			ModuloVRTAVAS400.LOG.trace("Fin de execute en clase ModuloVRTAVAS400");
		}
	 ...
	 */
	private BigDecimal moduloVRTAVAS400(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		//Variables locales
		BigDecimal vrtaVAS400 = BigDecimal.ZERO;
		BigDecimal varRever;
		BigDecimal varVzc;
		Modulo moduloVZC;
		Umic varUmic2;
		...
		if (ModuloVRTAVAS400.LOG.isTraceEnabled()) {
			ModuloVRTAVAS400.LOG.trace("Inicio función << ModuloVRTAVAS400 >> de la clase ModuloVRTAVAS400, para la iteracion = {}", iteracion);
		}
		...
		// Si el estado del asegurado 1 es anulado entonces vzc vale cero
		if (umic.getOtrosDatos().getCestadoAseg1().equals("A")) {
			varVzc = BigDecimal.ZERO;
		}else {
		...
		if (umic.getAsegurados().getFnacAseg2()!= null){
			...
			if (varUmic2 == null){
				...
				try {
					...
					ModuloVRTAVAS400.LOG.error(e.getMessage());
				}
			...
			vrtaVAS400 = varVzc.add(varRever.multiply(varVzc2.subtract(vaVzc2PorVarVzc)));
		} else {
			vrtaVAS400 = varVzc;
		}
		...
		if (ModuloVRTAVAS400.LOG.isTraceEnabled()) {
			ModuloVRTAVAS400.LOG.trace("Fin función << ModuloVRTAVAS400 >> de la clase ModuloVRTAVAS400, para la iteracion = {} con resultado vrtavas400 = {}", iteracion, vrtaVAS400);
		}
