 ...
 *
 */
public class ModuloCSP362J implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloCSP362J.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_CSP362J;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloCSP362J.LOG.isTraceEnabled()) {
				ModuloCSP362J.LOG.trace("Inicio de execute en clase ModuloCSP362");
			}
			...
			resultado = moduloCSP362J(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloCSP362J.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloCSP362J.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloCSP362J.LOG.isTraceEnabled()) {
			ModuloCSP362J.LOG.trace("Fin de execute en clase ModuloCSP362");
		}
	 ...
	 */
	private BigDecimal moduloCSP362J(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		if (ModuloCSP362J.LOG.isTraceEnabled()) {
			ModuloCSP362J.LOG.trace("Inicio función << moduloCSP362 >> de la clase ModuloCSP362, para la iteracion = {}", iteracion);
		}
		...
		if (ConstantsModulos.CTE_FIRST_ITER == iteracion ) {		
			csp362 = UtilModulos.setVarPUCCAPdifer(mapVariables, CLAVE_VAR_PUCCAPDIFER, FuncionesVBX.puccapDifer(varPU, varI1Bti, varI2Bti, varNR,
					...
					UtilModulos.getVarDiferCol(mapVariables, CLAVE_VAR_DIFER_COL, BigDecimal.ONE, varFechaEfecto, umic.getDatosDescuentos().getFecJubilacion(), varCriFec),
					varTCy));			
		} else {
			//Si proyUmic(j).fechaDesde  < fecRentaIni
				if (proyUmicJ.getFechaDesde().before(umic.getDatosDescuentos().getFecJubilacion())) {
		...
		if (ModuloCSP362J.LOG.isTraceEnabled()) {
			ModuloCSP362J.LOG.trace("Fin función << moduloCSP362 >> de la clase ModuloCSP362, para la iteracion = {}, con resultado csp362 = {}", iteracion, csp362);
		}
