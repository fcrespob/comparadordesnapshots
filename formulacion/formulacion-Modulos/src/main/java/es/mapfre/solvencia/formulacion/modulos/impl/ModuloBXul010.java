...
/**
 * Clase que implementa el modulo BXul010.
 ...
 * 				Bx(ul010, tcm+beta) = BGMORNL^Fall (tcm + beta) + BGMORNLGIP^Fall (tcm + beta)
 * @author apedro
 *
 */
public class ModuloBXul010 implements Modulo {


	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloBXul010.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_BXul010;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloBXul010.LOG.isTraceEnabled()) {
				ModuloBXul010.LOG.trace("Inicio de execute en clase ModuloBxul010");
			}
			
			//Recuperamos los datos que le pasaremos a la función moduloBxul010
			...
			//Se invoca a la función de calculo Bxul010
			resultado = moduloBxul010(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso, terminal);
			
		} catch (Solvencia2Excepcion e) {
			ModuloBXul010.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloBXul010.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloBXul010.LOG.isTraceEnabled()) {
			ModuloBXul010.LOG.trace("Fin de execute en clase ModuloBxul010");
		}
	 ...
	 */
	private TotalFlujoProyeccion moduloBxul010(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		//Variables locales
		BigDecimal varBxul010 = BigDecimal.ZERO;
		...
		if (ModuloBXul010.LOG.isTraceEnabled()) {
			ModuloBXul010.LOG.trace("Inicio función << moduloBXul010 >> de la clase ModuloBXul010, para la  iteracion = {}", iteracion);
		}
		...
		//Se realiza el cálculo de la fórmula bxul010(j) = varBgmornl + varBgmornlgip
		varBxul010 = varBgmornl.add(varBgmornlgip);
		
		if (ModuloBXul010.LOG.isTraceEnabled()) {
			ModuloBXul010.LOG.trace("Fin función << ModuloBxul010 >> de la clase ModuloBxul010, para la iteracion = {} con resultado varBxul010 = {}", iteracion, varBxul010);
		}
		
		//Se retorna bxul010 y los terminales
		salida.setProvbtiproy(varBxul010);
