 ...
 *
 */
public class ModuloPRI007SR implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI007SR.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI007SR;
	
	private static final String CLAVE_CRIFEC = ConstantsModulos.CTE_VA_CRIT_FEC;
	private static final String CLAVE_VAR_CRIFEC = CLAVE_CRIFEC.concat(CLAVE_MODULO);
	
	private static final String CLAVE_FEC_EFEC = ConstantsModulos.CTE_FEC_EFEC; 
	private static final String CLAVE_CALCULO_PRI007SR = "pri007sr";
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI007SR.LOG.isTraceEnabled()) {
				ModuloPRI007SR.LOG.trace("Inicio de execute en clase ModuloPRI007SR");
			}
			...
			resultado = moduloPRI007SR(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloPRI007SR.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI007SR.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloPRI007SR.LOG.isTraceEnabled()) {
			ModuloPRI007SR.LOG.trace("Fin de execute en clase ModuloPRI007SR");
		}
	 ...
	 */
	private BigDecimal moduloPRI007SR(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
			final Timestamp fcalc, final Umic umic, final DetalleBaseTecnica btcUmic, final Map<String, Object> mapVariables) {
		//Variables locales
		BigDecimal prim007sr = BigDecimal.ZERO;
		String varCriFec;
		...
		if (ModuloPRI007SR.LOG.isTraceEnabled()) {
			ModuloPRI007SR.LOG.trace("Inicio función << moduloPRI007SR >> de la clase ModuloPRI007SR, para la iteracion = {}", iteracion);
		}
		...
		if(umic.getPrimas().getCformpago() != ConstantsModulos.CTE_CFORMPAG_UNICA) {
			if (btcUmic.getBt().equals(ConstantsModulos.CTE_BT_N17LIRIN)) {
				prim007sr = umic.getPrimas().getIprimanetaini();
			}
			return prim007sr;
		}
		...
		final String clave = CLAVE_CALCULO_PRI007SR.concat("_").concat(String.valueOf(varNumAnualidades));
		if(!mapVariables.containsKey(CLAVE_ITERACION_PREVIA)) {
			...
			prim007sr = varPNA0.multiply(Util.pow(factorAnual, varNumAnualidades)).multiply(BigDecimal.ONE.divide(varNP, ConstantsFunciones.MATH_CONTEXT));
			mapVariables.put(clave, prim007sr);
		} else {
			if(mapVariables.containsKey(clave)) {
				prim007sr = (BigDecimal)mapVariables.get(clave);
			} else {
				final String clavePrevia = (String)mapVariables.put(CLAVE_ITERACION_PREVIA, clave);
				prim007sr = (BigDecimal)mapVariables.get(clavePrevia);
				...
				for(int i = varNumAnualidadesAnt; i < varNumAnualidades; i++) {
					prim007sr = prim007sr.multiply(factorAnual, ConstantsFunciones.MATH_CONTEXT);
				}
				mapVariables.put(clave, prim007sr);
			}
		}
		...
		if (ModuloPRI007SR.LOG.isTraceEnabled()) {
			ModuloPRI007SR.LOG.trace("Fin función << moduloPRI007SR >> de la clase ModuloPRI007SR, para la iteracion = {}, con resultado prim007sr = {}", iteracion, prim007sr);
		}
