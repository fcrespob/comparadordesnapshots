...
public class ModuloPRI007R implements Modulo {
	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI007R.class);
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI007R;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI007R.LOG.isTraceEnabled()) {
				ModuloPRI007R.LOG.trace("Inicio de execute en clase ModuloPRI007R");
			}
			...
			resultado = moduloPRI007R(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, codSubproceso,
					mapVariables);
		} catch (Solvencia2Excepcion e) {
			ModuloPRI007R.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI007R.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		if (ModuloPRI007R.LOG.isTraceEnabled()) {
			ModuloPRI007R.LOG.trace("Fin de execute en clase ModuloPRI007R");
		}
		return resultado;
	}
	...
	private BigDecimal moduloPRI007R(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente,
		...
		// variables locales
		BigDecimal pri007R = BigDecimal.ZERO;
		...
		if (ModuloPRI007R.LOG.isTraceEnabled()) {
			ModuloPRI007R.LOG.trace(
					"Inicio de la función << moduloPRI007R >> de la clase ModuloPRI007R, para la iteración = {}",
					iteracion);
		}
		...
		if(umic.getPrimas().getCformpago() != ConstantsModulos.CTE_CFORMPAG_UNICA){
			...
			pri007R = varPNA0.multiply((BigDecimal.ONE.add(varPRP.divide(ConstantsFunciones.CTE_OPER_100,ConstantsFunciones.MATH_CONTEXT))).pow(varTC).multiply(BigDecimal.ONE.add(varRPF.divide(ConstantsFunciones.CTE_OPER_100,ConstantsFunciones.MATH_CONTEXT))).multiply(varPDecimal.divide(varFPDecimal,ConstantsFunciones.MATH_CONTEXT)));
		}

		if (ModuloPRI007R.LOG.isTraceEnabled()) {
			ModuloPRI007R.LOG.trace(
					"Fin de la función << moduloPRI007R >> de la clase ModuloPRI007R, para la iteración = {}",
					iteracion);
		}
		return pri007R;
	}
