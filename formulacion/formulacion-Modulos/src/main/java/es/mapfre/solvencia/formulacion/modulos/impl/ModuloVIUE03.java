...
public class ModuloVIUE03 implements Modulo {
	
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI003R.class);

	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_VIUE03;	
	...
	public Object execute(Object... args) throws Solvencia2Excepcion {
		...
		BigDecimal cspviue03= BigDecimal.ZERO;
		
		try {
			if (ModuloVIUE03.LOG.isTraceEnabled()) {
				ModuloVIUE03.LOG.trace("Inicio de execute en clase ModuloVIUE03");
			}
			...
			cspviue03 = moduloVIUE03(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, codSubproceso, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloVIUE03.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloVIUE03.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
	
		if (ModuloVIUE03.LOG.isTraceEnabled()) {
			ModuloVIUE03.LOG.trace("Fin de execute en clase ModuloVIUE03");
		}
	...
	private BigDecimal moduloVIUE03(List<DetalleCorriente> proyUmic, BloqueCorriente bloqueCorriente, int iteracion,
			Timestamp fcalc, Umic umic, DetalleBaseTecnica btcUmic, String codSubproceso,
			Map<String, Object> mapVariables) {
		
		BigDecimal varviue03 = BigDecimal.ZERO;
		BigDecimal varFactorPrpss = BigDecimal.ZERO;
		String  varAnoEspStr, varPrpssStr, varE1PSJStr, varPorviussStr, varLimviuStr, varPORREDUC;
		BigDecimal varAnoEsp = BigDecimal.ONE;
		BigDecimal varPrpss;
		BigDecimal porreduc;
		BigDecimal varE1PSJ = BigDecimal.ONE; 
		BigDecimal varPorviuss = BigDecimal.ONE; 
		BigDecimal varLimviu = BigDecimal.ONE;
		...
		Fecha varFecJ;
		Integer varAnoj;
		BigDecimal varPSJj = BigDecimal.ZERO, varPSVj, varC1sant = null, max;
		String varPrpssStr2001, varPrpssStr2002 = null, varPrpssStrREST = null;
		...
		if (ModuloVIUE03.LOG.isTraceEnabled()) {
			ModuloVIUE03.LOG.trace("Inicio función << moduloVIUE03 >> de la clase ModuloVIUE03, para la  iteracion = {}", iteracion);
		}
		...
		varPct = ConstantsFunciones.CTE_OPER_50.divide(ConstantsFunciones.CTE_OPER_100);
		...
		if(null == varE1PSJStr){
			...
			varE1PSJ = new BigDecimal(varE1PSJStr);
			varE1PSJ = varE1PSJ.setScale(2, RoundingMode.HALF_UP);
			varE1PSJ = varE1PSJ.divide(new BigDecimal("100"));	
		...
		varLimviuStr = UtilModulos.getDatosEspecificosUmic(mapVariables, CLAVE_VAR_LIMITEVIU, umic.getDatosGenerales().getKpoliza(), umic.getDatosGenerales().getKsubpoliza(), umic.getDatosGenerales().getKcertificado(), umic.getDatosGenerales().getNsuscri(), CLAVE_LIMITEVIU);
		if(null == varLimviuStr){
			
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_DJ, new Object[]{CLAVE_LIMITEVIU});
			
		}else{
			
			varLimviu = UtilModulos.StringToBigDecimal(varLimviuStr);
		...
		if(varAnoEsp.equals(new BigDecimal(2001))){
			...
			varPSJj = porreduc.multiply(varE1PSJ).multiply(auxPRPSS2001).multiply(auxPRPSS2002).multiply(auxPRPSSREST.pow(varAnoj-varAnoEsp.intValue()-2));
			
		}else if(varAnoEsp.equals(new BigDecimal(2002))){
			auxPRPSS2002 = BigDecimal.ONE.add(varPrpss2002);
			auxPRPSSREST = BigDecimal.ONE.add(varPrpssREST);
			
			varPSJj = porreduc.multiply(varE1PSJ).multiply(auxPRPSS2002).multiply(auxPRPSSREST.pow(varAnoj-varAnoEsp.intValue()-1));
			
		}else if(varAnoEsp.compareTo(new BigDecimal(2002)) == 1){
			auxPRPSSREST = BigDecimal.ONE.add(varPrpssREST);
			
			varPSJj = porreduc.multiply(varE1PSJ).multiply(auxPRPSSREST.pow(varAnoj-varAnoEsp.intValue()));
			
		}
		
		varPSVj = varPorviuss.multiply(varPSJj);
		max = varPct.multiply(varPSJj.multiply(ConstantsFunciones.CTE_OPER_14).add(varC1sant.multiply(ConstantsFunciones.CTE_OPER_12)));
		if(varLimviu.compareTo(max)>0)
			max = varLimviu;
		varviue03 = max.subtract(varPSVj.multiply(ConstantsFunciones.CTE_OPER_14)).divide(ConstantsFunciones.CTE_OPER_12, ConstantsFunciones.MATH_CONTEXT);
								
		if (ModuloVIUE03.LOG.isTraceEnabled()) {
			ModuloVIUE03.LOG.trace("Fin de la función << moduloVIUE03 >> de la clase ModuloVIUE03, para la iteración = {}", iteracion);
		}
