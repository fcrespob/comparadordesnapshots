...
/**
 * Clase que implementa el modulo BXul001.
 ...
 * 				Bx(ul001, tcm + beta) = BGMORNL (tcm + beta) + B_VIDA (tcm + beta) + BGMORNLGIP (tcm + beta)
 * @author apedro
 *
 */
public class ModuloBXul001 implements Modulo {


	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloBXul001.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_BXul001;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloBXul001.LOG.isTraceEnabled()) {
				ModuloBXul001.LOG.trace("Inicio de execute en clase ModuloBxul001");
			}
			
			//Recuperamos los datos que le pasaremos a la función moduloBxul001
			...
			//Se invoca a la función de calculo Bxul001
			resultado = moduloBxul001(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloBXul001.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloBXul001.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloBXul001.LOG.isTraceEnabled()) {
			ModuloBXul001.LOG.trace("Fin de execute en clase ModuloBxul001");
		}
	 ...
	 */
	private TotalFlujoProyeccion moduloBxul001(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		BigDecimal varBgmornl;
		Integer nmeses;
		BigDecimal varBvida;
		//Fin variables locales
		
		if (ModuloBXul001.LOG.isTraceEnabled()) {
			ModuloBXul001.LOG.trace("Inicio función << moduloBXul001 >> de la clase ModuloBXul001, para la  iteracion = {}", iteracion);
		}
		...
		lstValoresQx = UtilModulos.getVarValoresTabMort(mapVariables, CLAVE_VAR_VAL_TAB_MORT_Q, umic, btcUmic, 
				IObtenerConfiguracion.OrdenAsegurado.ASEG1, varCriterioEdad, ConstantesSolvencia.CTE_TABMORT_Q);
		...
		varBgmornl = FuncionesAuxiliares.bgmornl(varX.intValue(), lstValoresTabMort, lstValoresQx, varn, varI1, varTcm, varCapital, varAlfa2, varAlfa230, varfactorAlfa2, varPeriocidad, mapVariables);
		
		varBvida = FuncionesAuxiliares.bvida(varX.intValue(), lstValoresTabMort, varn, varI1, varTcm, varAlfat, varAlfa2, varCapital);
		
		//Se realiza el cálculo de la fórmula bxul001(j) = varBgmornl + varBgmornlgip + varBvida
		varBxul001 = varBgmornl.add(varBgmornlgip).add(varBvida);
		
		if (ModuloBXul001.LOG.isTraceEnabled()) {
			ModuloBXul001.LOG.trace("Fin función << ModuloBxul001 >> de la clase ModuloBxul001, para la iteracion = {} con resultado varBxul001 = {}", iteracion, varBxul001);
		}
		
		//Se retorna bxul010 y los terminales
		salida.setProvbtiproy(varBxul001);
