...
public class ModuloFPTOVIU implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloFPTOVIU.class);

	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_FPTOVIU;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloFPTOVIU.LOG.isTraceEnabled()) {
				ModuloFPTOVIU.LOG.trace("Inicio de execute en clase FPTOVIU");
			}
			
			// Recuperamos los datos que le pasaremos a la función moduloFPTOVIU.
			...
			//Llamamos a la función moduloFPTOVIU.
			resultado = moduloFPTOVIU(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloFPTOVIU.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloFPTOVIU.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloFPTOVIU.LOG.isTraceEnabled()) {
			ModuloFPTOVIU.LOG.trace("Fin de execute en clase FPTOVIU");
		}
	 ...
	 */
	private BigDecimal moduloFPTOVIU(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		BigDecimal fptoviu = BigDecimal.ZERO;
		...
		if (ModuloFPTOVIU.LOG.isTraceEnabled()) {
			ModuloFPTOVIU.LOG.trace("Inicio función << moduloFPTOZC >> de la clase moduloFPTOZC, para la iteracion = {}", iteracion);
		}
...
//INI-TAR00400971
		if ((umic.getDatosGenerales().getKprestacion() != null) && (umic.getDatosGenerales().getKprestacion().equals("RS901"))) {
			varFracc1 = FuncionesAuxiliares.nAnnos(fcalc,
			 	proyUmic.get(iteracion).getFechaDesde(), varCriterFec);	
		}
//FIN-TAR00400971		
		...
		if (iteracion == ConstantsModulos.CTE_FIRST_ITER){
			...
			fptoviu = (varLzc.subtract(varLj)).divide(varLzc, ConstantsFunciones.MATH_CONTEXT);
			...
			if (fecDevAnt == null){
				...
				if(null == fecDevAnt ){
					fecDevAnt = proyUmic.get(iteracion-2).getFechaDesde();
				}
			}
			...
			//
			fptoviu = (varLJant.subtract(varLj)).divide(varLzc, ConstantsFunciones.MATH_CONTEXT);
		...
		if (ModuloFPTOVIU.LOG.isTraceEnabled()) {
			ModuloFPTOVIU.LOG.trace("Fin función << moduloFPTOZC >> de la clase moduloFPTOZC, para la iteracion = {}, con resultado fptoviu = {}", iteracion, fptoviu);
		}
