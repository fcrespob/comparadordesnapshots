 ...
 *
 */
public class ModuloPRI008SR implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI008SR.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI008SR;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI008SR.LOG.isTraceEnabled()) {
				ModuloPRI008SR.LOG.trace("Inicio de execute en clase ModuloPRI008SR");
			}
			...
			resultado = moduloPRI008SR(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloPRI008SR.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI008SR.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloPRI008SR.LOG.isTraceEnabled()) {
			ModuloPRI008SR.LOG.trace("Fin de execute en clase ModuloPRI008SR");
		}
	 ...
	 */
	private BigDecimal moduloPRI008SR(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
			final Timestamp fcalc, final Umic umic, final DetalleBaseTecnica btcUmic, final Map<String, Object> mapVariables) {
		//Variables locales
		BigDecimal prim008sr = BigDecimal.ZERO;
		BigDecimal varPNA0;
		BigDecimal varPRP;
		BigDecimal varRPF;
		...
		if (ModuloPRI008SR.LOG.isTraceEnabled()) {
			ModuloPRI008SR.LOG.trace("Inicio función << moduloPRI008SR >> de la clase ModuloPRI008SR, para la iteracion = {}", iteracion);
		}
		...
		if(umic.getPrimas().getCformpago() != ConstantsModulos.CTE_CFORMPAG_UNICA) {
			if (btcUmic.getBt().equals(ConstantsModulos.CTE_BT_N17LIRIN)) {
				prim008sr = umic.getPrimas().getIprimanetaini();
			}
			return prim008sr;
		}
		...
		BigDecimal opVarFP = BigDecimal.ONE.divide(varFP, ConstantsFunciones.MATH_CONTEXT); //((1+ varRPF)/varFP)
		
		prim008sr = varPNA0.multiply(opVarPRP).multiply(opVarFP);
			

		if (ModuloPRI008SR.LOG.isTraceEnabled()) {
			ModuloPRI008SR.LOG.trace("Fin función << moduloPRI008SR >> de la clase ModuloPRI008SR, para la iteracion = {}, con resultado prim008sr = {}", iteracion, prim008sr);
		}
