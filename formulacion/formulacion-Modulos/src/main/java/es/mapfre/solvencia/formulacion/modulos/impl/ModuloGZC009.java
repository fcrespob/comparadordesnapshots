...
/**
 * Clase que implementa el modulo GZC009.
 ...
 * 		GZC(009,zc) = GZC003 + GZC001 
 *
 * @author apedro
 *
 */
public class ModuloGZC009 implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloGZC009.class);


	@Override
	public String getNombreServicio() {
		return ConstantsFactorias.MODULO_GZC009;
	}
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloGZC009.LOG.isTraceEnabled()) {
				ModuloGZC009.LOG.trace("Inicio de execute en clase ModuloGZC009");
			}
			
			//Recuperamos los datos que le pasaremos a la función moduloGZC009
			...
			//Invocamos a la función de calculo GZC009
			resultado = moduloGZC009(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloGZC009.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloGZC009.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloGZC009.LOG.isTraceEnabled()) {
			ModuloGZC009.LOG.trace("Fin de execute en clase ModuloGZC009");
		}
	 ...
	 */
	private BigDecimal  moduloGZC009(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		//Variables locales
		BigDecimal gzc009 = BigDecimal.ZERO;
		...
		if (ModuloGZC009.LOG.isTraceEnabled()) {
			ModuloGZC009.LOG.trace("Inicio función << moduloGZC009 >> de la clase ModuloGZC009, para la iteracion = {}", iteracion);
		}
		...
		/**
			 * Para todos los periodos los cálculos se realizan de la misma manera (en la primera iteración los auxiliares son calculados las demás los recupera).
					...
					GZC009 (j) = GZC003 (j) + GZC001(j)
					
					Finalmente asignaremos la cuantía nominal del periodo como: 
					
					proyUmic(j).impFlujoNominal = GZC009 (j)
		...
		gzc009 = impFNGZC003.add(impFNGZC001);
		
		if (ModuloGZC009.LOG.isTraceEnabled()) {
			ModuloGZC009.LOG.trace("Fin función << moduloGZC009 >> de la clase ModuloGZC009, para la iteracion = {} con resultado GZC009 = {}", iteracion, gzc009);
		}
