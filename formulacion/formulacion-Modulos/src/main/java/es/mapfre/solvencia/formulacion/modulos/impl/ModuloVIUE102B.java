...
public class ModuloVIUE102B implements Modulo {
	
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloVIUE102B.class);

	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_VIUE102B;	
	...
	public Object execute(Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloVIUE102B.LOG.isTraceEnabled()) {
				ModuloVIUE102B.LOG.trace("Inicio de execute en clase ModuloVIUE102B");
			}
			...
			viue102 = moduloVIUE102B(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, codSubproceso, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloVIUE102B.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloVIUE102B.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
	
		if (ModuloVIUE102B.LOG.isTraceEnabled()) {
			ModuloVIUE102B.LOG.trace("Fin de execute en clase ModuloVIUE102B");
		}
	...
	private BigDecimal moduloVIUE102B(List<DetalleCorriente> proyUmic, BloqueCorriente bloqueCorriente, int iteracion,
		...
		if (ModuloVIUE102B.LOG.isTraceEnabled()) {
			ModuloVIUE102B.LOG.trace("Inicio función << moduloVIUE102B >> de la clase ModuloVIUE102, para la  iteracion = {}", iteracion);
		}
		...
		if(null == varEGARBStr){
			...
			varEGARBbis = varEGARB.multiply(BigDecimal.valueOf(0.75));
		...
		if(varAnoEsp.equals(new BigDecimal(2001))){
			...
			varviue102b = varEGARBbis.subtract(varE1PSV.multiply(auxPRPSS2001).multiply(auxPRPSS2002).multiply(auxPRPSSREST.pow(varAnoj-varAnoEsp.intValue()-2)));
			
		}else if(varAnoEsp.equals(new BigDecimal(2002))){
			auxPRPSS2002 = BigDecimal.ONE.add(varPrpss2002);
			auxPRPSSREST = BigDecimal.ONE.add(varPrpssREST);
			
			varviue102b = varEGARBbis.subtract(varE1PSV.multiply(auxPRPSS2002).multiply(auxPRPSSREST.pow(varAnoj-varAnoEsp.intValue()-1)));
			
		}else if(varAnoEsp.compareTo(new BigDecimal(2002)) == 1){
			auxPRPSSREST = BigDecimal.ONE.add(varPrpssREST);
			
			varviue102b = varEGARBbis.subtract(varE1PSV.multiply(auxPRPSSREST.pow(varAnoj-varAnoEsp.intValue())));
			
		}	
		
		if (varviue102b.compareTo(BigDecimal.ZERO) < 0 ) {
			varviue102b = BigDecimal.ZERO;
		}
		
		if (ModuloVIUE102B.LOG.isTraceEnabled()) {
			ModuloVIUE102B.LOG.trace("Fin de la función << moduloVIUE102B >> de la clase ModuloVIUE102B, para la iteración = {}", iteracion);
		}
