...
public class ModuloRTPMAT implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloRTPMAT.class);
	
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_RTPMAT;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloRTPMAT.LOG.isTraceEnabled()) {
				ModuloRTPMAT.LOG.trace("Inicio de execute en clase ModuloRTPMAT");
			}
			...
			resultado = moduloRTPMAT(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloRTPMAT.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloRTPMAT.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloRTPMAT.LOG.isTraceEnabled()) {
			ModuloRTPMAT.LOG.trace("Fin de execute en clase ModuloRT010");
		}
	 ...
	 */
	private BigDecimal moduloRTPMAT(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		if (ModuloRTPMAT.LOG.isTraceEnabled()) {
			ModuloRTPMAT.LOG.trace("Inicio función << ModuloRTPMAT >> de la clase ModuloRTPMAT, para la iteracion = {}", iteracion);
		}
		...
		if (ModuloRTPMAT.LOG.isTraceEnabled()) {
			ModuloRTPMAT.LOG.trace("Fin función << ModuloRTPMAT >> de la clase ModuloRTPMAT, para la iteracion = {} con resultado rtpmat = {}", iteracion, rtpmat);
		}
