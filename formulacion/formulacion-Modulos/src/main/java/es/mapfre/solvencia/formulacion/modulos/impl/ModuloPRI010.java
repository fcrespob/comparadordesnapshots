 ...
 *
 */
public class ModuloPRI010 implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI010.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI010;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI010.LOG.isTraceEnabled()) {
				ModuloPRI010.LOG.trace("Inicio de execute en clase ModuloPRI010");
			}
			...
			resultado = moduloPRI010(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloPRI010.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI010.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloPRI010.LOG.isTraceEnabled()) {
			ModuloPRI010.LOG.trace("Fin de execute en clase ModuloPRI010");
		}
	 ...
	 */
	private BigDecimal moduloPRI010(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
			final Timestamp fcalc, final Umic umic, final DetalleBaseTecnica btcUmic, final Map<String, Object> mapVariables) {
		//Variables locales
		BigDecimal prim010 = BigDecimal.ZERO;
		BigDecimal varPNA0;
		BigDecimal varPRP;
		BigDecimal varRPF;
		...
		if (ModuloPRI010.LOG.isTraceEnabled()) {
			ModuloPRI010.LOG.trace("Inicio función << moduloPRI010 >> de la clase ModuloPRI010, para la iteracion = {}", iteracion);
		}
		...
		if (fechaPago.getMes() == fechaProxRenovacion.getMes()) {
			...
			prim010 = varPNA0.multiply(opVarPRP).multiply(opVarNP);
		}
		
//		System.out.println(iteracion + ";" + varPNA0 + ";" + varPRP + ";" + varNumAnualidades + ";" + varRPF + ";" + varNP + ";" + prim010 + ";" + opVarNP + ";" + opVarPRP);

		if (ModuloPRI010.LOG.isTraceEnabled()) {
			ModuloPRI010.LOG.trace("Fin función << moduloPRI010 >> de la clase ModuloPRI010, para la iteracion = {}, con resultado prim010 = {}", iteracion, prim010);
		}
