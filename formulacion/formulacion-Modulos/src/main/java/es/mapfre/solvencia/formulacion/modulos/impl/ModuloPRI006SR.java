 ...
 *
 */
public class ModuloPRI006SR implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI006SR.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI006SR;
	...
	private static final String CLAVE_CALCULO_PRI006SR = "pri006sr";
	private static final String CLAVE_OPERADOR_RPF_NP = "operadorRPF_NP";
	private static final String CLAVE_FACTOR_ANUAL = "factorAnualPRI006SR";
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI006SR.LOG.isTraceEnabled()) {
				ModuloPRI006SR.LOG.trace("Inicio de execute en clase ModuloPRI006SR");
			}
			...
			resultado = moduloPRI006SR(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloPRI006SR.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI006SR.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloPRI006SR.LOG.isTraceEnabled()) {
			ModuloPRI006SR.LOG.trace("Fin de execute en clase ModuloPRI006SR");
		}
	 ...
	 */
	private BigDecimal moduloPRI006SR(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
			final Timestamp fcalc, final Umic umic, final DetalleBaseTecnica btcUmic, final Map<String, Object> mapVariables) {
		//Variables locales
		BigDecimal prim006sr = BigDecimal.ZERO;
		String varCriFec;
		...
		if (ModuloPRI006SR.LOG.isTraceEnabled()) {
			ModuloPRI006SR.LOG.trace("Inicio función << moduloPRI006SR >> de la clase ModuloPRI006SR, para la iteracion = {}", iteracion);
		}
		
		//Validamos los parametros de entrada
		ValidacionesComunesModulos.validarParamEntrada(proyUmic, bloqueCorriente, fcalc, umic, btcUmic);
		
		// Si no tiene fecha de pago
		if(bloqueCorriente.getFechaPago() == null || bloqueCorriente.getFechaPago().after(umic.getFechas().getFecefecfin()) || 
		...
		if (umic.getPrimas().getCformpago().equals(ConstantsModulos.CTE_CFORMPAG_UNICA)) {
			if (btcUmic.getBt().equals(ConstantsModulos.CTE_BT_N17LIRIN)) {
				prim006sr = umic.getPrimas().getIprimanetaini();
			}
			return prim006sr;
		}
		...
		final String clave = CLAVE_CALCULO_PRI006SR.concat("_").concat(String.valueOf(potencia));
		...
		if(!mapVariables.containsKey(clave)) {
			if (!mapVariables.containsKey(CLAVE_FACTOR_ANUAL)){
				...
				//[((1+varRPF/100  ))/varNP]
				operadorRpfNp = BigDecimal.ONE.divide(varNP, ConstantsFunciones.MATH_CONTEXT);
			...
			prim006sr = varPNATC.multiply(Util.pow(factorAnual, potencia)).multiply(operadorRpfNp);
			mapVariables.put(clave, prim006sr);
			
		} else {
			prim006sr = (BigDecimal)mapVariables.get(clave);
		}
		
		if (ModuloPRI006SR.LOG.isTraceEnabled()) {
			ModuloPRI006SR.LOG.trace("Fin función << moduloPRI006SR >> de la clase ModuloPRI006SR, para la iteracion = {}, con resultado prim006sr = {}", iteracion, prim006sr);
		}
