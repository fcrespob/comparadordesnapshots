 ...
 *
 */
public class ModuloPRI307b implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI307b.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI307B;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI307b.LOG.isTraceEnabled()) {
				ModuloPRI307b.LOG.trace("Inicio de execute en clase PRI307b");
			}
			...
			resultado = moduloPRI307b(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloPRI307b.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI307b.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloPRI307b.LOG.isTraceEnabled()) {
			ModuloPRI307b.LOG.trace("Fin de execute en clase PRI307b");
		}
	 ...
	 */
	private BigDecimal moduloPRI307b(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		BigDecimal pri307b = BigDecimal.ZERO;
		BigDecimal varPNA0;
		BigDecimal varPRP;
		...
		if (ModuloPRI307b.LOG.isTraceEnabled()) {
			ModuloPRI307b.LOG.trace("Inicio función << moduloPRI307b >> de la clase ModuloPRI307b, para la iteracion = {}", iteracion);
		}
		...
		varRPF = umic.getPrimas().getPrecargfrac().multiply(ConstantsFunciones.CTE_OPER_0_PUNTO_01, ConstantsFunciones.MATH_CONTEXT);
		...
		if ((fechaPago.getMes() == fechaProxRenovacion.getMes()) || 
			...
			if (opVarGE == null){
				...
				opVarRPF = BigDecimal.ONE.add(varRPF).divide(varFP,ConstantsFunciones.MATH_CONTEXT);
			...
			varPNA = varPNA0.multiply(opVarPRP.pow(varNumAnualidades)).multiply(opVarRPF);
			...
			// Calculo PRI307b
			pri307b = calcularPri307b(bloqueCorriente.getFechaPago(), varAntRenova,varProxRenova,varfecvcto,varTipFlxPrima,varPNA,varFP,varCriterioFecha,varNmeses, bloqueCorriente.getFechaDevengo(), umic.getFechas().getFecfinpagprim());
			
		}
			
		if (ModuloPRI307b.LOG.isTraceEnabled()) {
			ModuloPRI307b.LOG.trace("Fin función << moduloPRI307b >> de la clase ModuloPRI307b, para la iteracion = {}, con resultado prim307b = {}", iteracion, pri307b);
		}
	...
	private BigDecimal calcularPri307b(final Timestamp fecPago, final Timestamp varAntRenova, final Timestamp varProxRenova,
		...
		if (ModuloPRI307b.LOG.isTraceEnabled()) {
			ModuloPRI307b.LOG.trace("Inicio de la función << calcularPrim307b >> de la clase ModuloPRI307b");
		}		
			...
			if (varTipFlxPrima.equals("A")) {
				...
				if (varFP.equals(BigDecimal.ONE) || varFP.equals(BigDecimal.valueOf(ConstantsFunciones.CTE_2))) {
					...
					if (fechaPago.getMes() == fecvcto.getMes()) {
						pri307b = varPNA.multiply(varFP.subtract(BigDecimal.valueOf(varNumRec)));
					}
					else {
						pri307b = varPNA;
					}
				...
				if (varFP.equals(BigDecimal.ONE)) {
					...
					pri307b = BigDecimal.valueOf(varD1).divide(ConstantsFunciones.CTE_OPER_365, ConstantsFunciones.MATH_CONTEXT);
				}
				else {
					...
					pri307b = varPrimRec.multiply(BigDecimal.valueOf(varNumRec).add(BigDecimal.valueOf(varD1).divide(BigDecimal.valueOf(varD2))));
				}
			}
		...
		if (ModuloPRI307b.LOG.isTraceEnabled()) {
			ModuloPRI307b.LOG.trace("Fin de la función << calcularPrim307b>> de la clase ModuloPRI307b");
		}
