 ...
 *
 */
public class ModuloPRI001SR implements Modulo{
	
	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI001SR.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI001SR;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI001SR.LOG.isTraceEnabled()) {
				ModuloPRI001SR.LOG.trace("Inicio de execute en clase ModuloPRI001SR");
			}
			...
			resultado = moduloPRI001SR(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, codSubproceso, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloPRI001SR.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI001SR.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
	
		if (ModuloPRI001SR.LOG.isTraceEnabled()) {
			ModuloPRI001SR.LOG.trace("Fin de execute en clase ModuloPRI001SR");
		}
	 ...
	 */
	private BigDecimal moduloPRI001SR (final List<DetalleCorriente> proyUmic       ,
		...
		BigDecimal           pri001sr = BigDecimal.ZERO;
		BigDecimal 			 varP;
		...
		if (ModuloPRI001SR.LOG.isTraceEnabled()) {
			ModuloPRI001SR.LOG.trace("Inicio función << moduloPRI001SR >> de la clase ModuloPRI001SR, para la  iteracion = {}", iteracion);
		}
		...
		if (null == bloqueCorriente.getFechaPago()) {
			...
			if (ModuloPRI001SR.LOG.isTraceEnabled()) {
				ModuloPRI001SR.LOG.trace("Fin función << moduloPRI001SR >> de la clase ModuloPRI001SR, para la  iteracion = {}, con resultado pri001sr = {}", iteracion, pri001sr);
			}
			...
			if (umic.getPrimas().getCformpago().equals(ConstantsModulos.CTE_CFORMPAG_UNICA) ) {
				if (btcUmic.getBt().equals(ConstantsModulos.CTE_BT_N17LIRIN)) {
					pri001sr = umic.getPrimas().getIprimanetaini();
				}
				...
				if(bloqueCorriente.getFechaPago().after(umic.getFechas().getFecefecfin())){
					...
					pri001sr = varFallcj.multiply(calculoAux.divide(varDenominador, ConstantsFunciones.MATH_CONTEXT));
				}else{
				
					pri001sr = umic.getPrimas().getIprimatarada().multiply(BigDecimal.ONE.divide(varNP, ConstantsFunciones.MATH_CONTEXT));
