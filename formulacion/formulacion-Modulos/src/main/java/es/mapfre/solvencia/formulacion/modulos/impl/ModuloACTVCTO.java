 ...
 *
 */
public class ModuloACTVCTO implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloACTVCTO.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_ACTVCTO;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			...
			if (ModuloACTVCTO.LOG.isTraceEnabled()) {
				ModuloACTVCTO.LOG.trace("Inicio de execute en clase ModuloACTVCTO");
			}
			
			//Recuperamos los datos que le pasaremos a la función moduloACTVCTO
			...
			//Invocamos a la función de calculo moduloACTVCTO
			resultado = moduloACTVCTO(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloACTVCTO.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloACTVCTO.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloACTVCTO.LOG.isTraceEnabled()) {
			ModuloACTVCTO.LOG.trace("Fin de execute en clase ModuloACTVCTO");
		}
	...
	/**
	 * El módulo ACTVCTO determina el cálculo de los factores de actualización financiera para la garantía  
	 * en un periodo dado así como el Flujo Actualizado de dicho periodo.
	 *  
	 * @param proyUmic
	 ...
	 */
	private BigDecimal moduloACTVCTO(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		//Variables locales
		BigDecimal actvcto = BigDecimal.ZERO;
		...
		if (ModuloACTVCTO.LOG.isTraceEnabled()) {
			ModuloACTVCTO.LOG.trace("Inicio función << moduloACTVCTO >> de la clase ModuloACTVCTO, para la iteracion = {}", iteracion);
		}
		...
		if (ConstantsModulos.CTE_PROY_PRV.equals(codSubproceso)) {
			...
			varFecha = bloqueCorriente.getFechaDevengo();
		}
			...
			if (null == varI2 || varI2.equals(BigDecimal.ZERO)) {
				...
				if (!fcalc.after(varffincas1)){
					...
					if(!varFecha.after(varffincas1)){
						...
						atcfinJ = Util.pow(BigDecimal.ONE.add(varI2.multiply(CTE_0_PUNTO_01, ConstantsFunciones.MATH_CONTEXT)), varAnoJ.negate());
		...
		if (ModuloACTVCTO.LOG.isTraceEnabled()) {
			ModuloACTVCTO.LOG.trace("Fin función << moduloACTVCTO >> de la clase ModuloACTVCTO, para la iteracion = {}, con resultado capitalCartera = {}", iteracion, actvcto);
		}
