 ...
 *
 */
public class ModuloPRI001BSR implements Modulo{

	private static final Logger LOG = LoggerFactory.getLogger(ModuloPRI001BSR.class);
	
	// Inicio de las variables estáticas para agilizar operaciones.
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_PRI001BSR;
	...
	public Object execute(Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloPRI001BSR.LOG.isTraceEnabled()) {
				ModuloPRI001BSR.LOG.trace("Inicio de execute en clase ModuloPRI001BSR");
			}
			...
			resultado = moduloPRI001BSR(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, codSubproceso, mapVariables);
			
		} catch (Solvencia2Excepcion e) {
			ModuloPRI001BSR.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloPRI001BSR.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
	
		if (ModuloPRI001BSR.LOG.isTraceEnabled()) {
			ModuloPRI001BSR.LOG.trace("Fin de execute en clase ModuloPRI001BSR");
		}
	 ...
	 */
	private BigDecimal moduloPRI001BSR(List<DetalleCorriente> proyUmic, 
		...
		BigDecimal           pri001bsr = BigDecimal.ZERO;
		BigDecimal 			 varP;
		BigDecimal 	 		 varGic;
		BigDecimal 	 		 varGE;
		...
		if (ModuloPRI001BSR.LOG.isTraceEnabled()) {
			ModuloPRI001BSR.LOG.trace("Inicio función << moduloPRI001BSR >> de la clase ModuloPRI001BSR, para la  iteracion = {}", iteracion);
		}
		...
		if (bloqueCorriente.getFechaPago().equals(null)){
			...
			if (ModuloPRI001BSR.LOG.isTraceEnabled()) {
				ModuloPRI001BSR.LOG.trace("Fin función << moduloPRI001BSR >> de la clase ModuloPRI001BSR, para la iteracion = {}, con resultado pri001bsr = {}", iteracion, pri001bsr);
			}
			...
			if(bloqueCorriente.getFechaPago().after(umic.getFechas().getFecefecfin())){
				...
				pri001bsr = varCapGar.multiply(varTarifa).multiply(varGF);
				
			} else {
				pri001bsr = umic.getPrimas().getIprimatarada();
			}
