...
public class ModuloRTPMATKS implements Modulo {

	/**
	 * Cte para log.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModuloRTPMATKS.class);
	
	private static final String CLAVE_MODULO = ConstantsFactorias.MODULO_RTPMATKS;
	...
	public Object execute(final Object... args) throws Solvencia2Excepcion {
		...
		try {
			if (ModuloRTPMATKS.LOG.isTraceEnabled()) {
				ModuloRTPMATKS.LOG.trace("Inicio de execute en clase ModuloRTPMAT");
			}
			...
			resultado = moduloRTPMATKS(proyUmic, bloqueCorriente, iteracion, fcalc, umic, btcUmic, mapVariables, codSubproceso);
			
		} catch (Solvencia2Excepcion e) {
			ModuloRTPMATKS.LOG.error(e.getIncidencia().getTextoError(), e);
			throw e;
		} catch (Exception e) {
			ModuloRTPMATKS.LOG.error(e.getMessage(), e);
			throw Solvencia2ExcepcionHelper.crearExcepcion(ConstantsFunciones.CTE_COD_ERROR_000, null, e);
		}
		
		if (ModuloRTPMATKS.LOG.isTraceEnabled()) {
			ModuloRTPMATKS.LOG.trace("Fin de execute en clase ModuloRT010");
		}
	 ...
	 */
	private BigDecimal moduloRTPMATKS(final List<DetalleCorriente> proyUmic, final BloqueCorriente bloqueCorriente, final Integer iteracion,
		...
		BigDecimal rtpmat = BigDecimal.ZERO;
		...
		if (ModuloRTPMATKS.LOG.isTraceEnabled()) {
			ModuloRTPMATKS.LOG.trace("Inicio función << ModuloRTPMAT >> de la clase ModuloRTPMAT, para la iteracion = {}", iteracion);
		}
		...
		if (bloqueCorriente.getFechaDevengo() != null && varK1 != null){
			...
			rtpmat = varProvMat.multiply(varK1);
		}	
		
		if (ModuloRTPMATKS.LOG.isTraceEnabled()) {
			ModuloRTPMATKS.LOG.trace("Fin función << ModuloRTPMAT >> de la clase ModuloRTPMAT, para la iteracion = {} con resultado rtpmat = {}", iteracion, rtpmat);
		}
