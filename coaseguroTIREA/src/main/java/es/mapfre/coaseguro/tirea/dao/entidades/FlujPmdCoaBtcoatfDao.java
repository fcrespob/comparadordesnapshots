...
public class FlujPmdCoaBtcoatfDao extends DaoBase{

	private static final String CACHE_NAME = "FLUJPMDCOABTCOATF";
	
	private static final int BATCH_SIZE = 1000;


	public FlujPmdCoaBtcoatfDao() {
		super();
		super.setCacheName(CACHE_NAME);
	}

	public Set<Entry<FlujPmdCoaBtcoatfKey, FlujPmdCoaBtcoatf>> entrySet() {
		return (Set<Entry<FlujPmdCoaBtcoatfKey, FlujPmdCoaBtcoatf>>) this.getCache().getAll(getCache().getKeys());
	}

	public FlujPmdCoaBtcoatf get(Object key) {
		if (key instanceof FlujPmdCoaBtcoatfKey) {
			return (FlujPmdCoaBtcoatf) getCache().get(key).getObjectValue();
		}
		return null;
	}

	public List<FlujPmdCoaBtcoatfKey> keySet() {
		return this.getCache().getKeys();
	}

	public FlujPmdCoaBtcoatf put(FlujPmdCoaBtcoatfKey key, FlujPmdCoaBtcoatf value) {
	...
	protected Comparator exportOrdered() {
		return new Comparator<FlujPmdCoaBtcoatfKey>() {
			@Override
			public int compare(FlujPmdCoaBtcoatfKey dc1, FlujPmdCoaBtcoatfKey dc2) {
	...
	public void loadCache(BeanIOReader reader) {
		FlujPmdCoaBtcoatf flujPmdCoaBtcoatf = null;
		Set<Element> valores = new HashSet<Element>();
		int bloque = 0;
		while ((flujPmdCoaBtcoatf = (FlujPmdCoaBtcoatf) reader.read()) != null) {
			valores.add(new Element(flujPmdCoaBtcoatf.getKey(), flujPmdCoaBtcoatf));
			bloque++;
	...
	public List<FlujPmdCoaBtcoatf> getValues() {

		Collection<Element> fr = this.values();
		List<FlujPmdCoaBtcoatf> lista = new ArrayList<FlujPmdCoaBtcoatf>();
		...
		while (itFR.hasNext()) {
			lista.add((FlujPmdCoaBtcoatf) itFR.next().getObjectValue());
		}
