...
public class FlujPMdCoaDao extends DaoBase{
	...
	public Set<Entry<FlujPMdCoaKey, FlujPMdCoa>> entrySet() {
		return (Set<Entry<FlujPMdCoaKey, FlujPMdCoa>>) this.getCache().getAll(getCache().getKeys());
	}

	public FlujPMdCoa get(Object key) {
		if (key instanceof FlujPMdCoaKey) {
			return (FlujPMdCoa) getCache().get(key).getObjectValue();
		}
		return null;
	}

	public List<FlujPMdCoaKey> keySet() {
	...
	public void loadCache(BeanIOReader reader) {
		FlujPMdCoa flujPMdCoa = null;
		Set<Element> valores = new HashSet<Element>();
		int bloque = 0;
		while ((flujPMdCoa = (FlujPMdCoa) reader.read()) != null) {
			valores.add(new Element(flujPMdCoa.getKey(), flujPMdCoa));
			bloque++;
	...
	public List<FlujPMdCoa> getValues() {

		Collection<Element> fr = this.values();
		List<FlujPMdCoa> lista = new ArrayList<FlujPMdCoa>();
		...
		while (itFR.hasNext()) {
			lista.add((FlujPMdCoa) itFR.next().getObjectValue());
		}
